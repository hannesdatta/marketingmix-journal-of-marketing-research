...
for (.v in c('lnusales','lnusales_hat','dlnusales', 'dlnusales_hat')) {
  predictions[, paste0(.v, '_mc'):=as.numeric(NA)]
  predictions[, paste0(.v, '_std'):=as.numeric(NA)]
  try(predictions[, paste0(.v, '_mc') := get(.v) - mean(get(.v),na.rm=T), by= c('type','category','country','brand', 'estim_set')],silent=T)
  try(predictions[, paste0(.v, '_std') := (get(.v) - mean(get(.v),na.rm=T))/sd(get(.v),na.rm=T), by= c('type','category','country','brand', 'estim_set')],silent=T)
...
for (ty in c('individual','omnibus')) {
 ...
 summary(r2s[N>=5*12&type=='ec_main_holdout'&estim_set==F]$R2_lev)
  
  hist(r2s[N>=5*12&type=='ec_main_holdout'&estim_set==F]$R2_lev)
 
 
 
  #r2
  r2s[estim_set==F, list(median(cor_lev)^2),by= c('type')]
  
  
 r2s=predictions[type%in%sel_ord, list(R2_diff = cor(dlnusales, dlnusales_hat, use='pairwise')^2,
                         R2_lev = cor(exp(lnusales), exp(lnusales_hat), use='pairwise')^2,
                         MSE_diff = mean((dlnusales_hat-dlnusales)^2,na.rm=T),
                         MSE_lev = mean((exp(lnusales_hat)-exp(lnusales))^2,na.rm=T)),
               by = c('type','category','country','brand', 'estim_set')]
  ...
  r2s=predictions[type%in%sel_ord, list(R2_diff = cor(dlnusales_std, dlnusales_hat_std, use='pairwise')^2,
                         R2_lev = cor(lnusales_std, lnusales_hat_std, use='pairwise')^2,
                         MSE_diff = mean((dlnusales_hat_std-dlnusales_std)^2,na.rm=T),
                         MSE_lev = mean((lnusales_hat_std-lnusales_std)^2,na.rm=T)),
               by = c('type', 'estim_set')]
  }
  ...
  tmp = r2s[, lapply(.SD, median,na.rm=T), by = c('type2','model_type', 'metric_type'), .SDcols=grep("R2|MSE|cor",colnames(r2s),value=T)]
  
  tmp[,cor_diffsq:=cor_diff^2]
  tmp[,cor_levsq:=cor_lev^2]
...
# - Text: R2 from (e.g., with mmix)
# - Holdout sample: Main MSE w/ log log & log log with only LDV
# - Terugschalen naar unit sales ipv log unit sales: how to?!
# - try out with 10% (holdout sample size)

# - Main results w/ competitive effects

# - Sorteren op squared errors
# - Std. predictions ipv mean-centering
...
# 10% vs. 20% vs. 25%
# median
# standardization of mean-centered values
...
r2=predictions[type=='ec_main_holdout', list(r2=cor(lnusales, lnusales_hat, use='pairwise')), by = c('category','country','brand', 'estim_set')]

setorder(r2, r2)


tmp = predictions[type=='ec_main_holdout' & brand =='samsung' & category=='phones_smart'&country=='china']

tmp = predictions[type=='ec_main_holdout' & brand =='lenovo' & category=='laptop'&country=='south korea']

tmp = predictions[type=='ec_main_holdout' & brand =='bajaj' & category=='microwave'&country=='india']

tmp = predictions[type=='ec_main_holdout' & brand =='telefunken' & category=='tv_gen1_crtv'&country=='australia']
