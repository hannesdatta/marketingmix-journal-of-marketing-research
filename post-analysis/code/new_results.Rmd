...
tmp=unique(elast, by='country')[, c('country','gdppercapitacurrent2010','gdpgrowth2010', 'ginicoef','pdi', 'uai','mas'),with=F]
...
print(kable(sanitize_table(tmp3), digits=3, caption = tab(paste0('Awesome title'), prefix=''), format='html', initial.zero = FALSE) %>% add_header_above(list(' '=1, 'Average Elasticities' = 3, 'Expenditure ratios'=2, 'Weights' = 1, 'Expenditure index' = 2)) %>% kable_styling() %>% footnote(number='Average elasticities are weighted by inverse standard errors of the brand- and category-specific effects. Expenditure ratios are calculated in line with Farley and Lehmann (1994, p. 119). Weights are equal to a country\'s GDP (in bn. USD; GDP per capita x population size in 2010, our sample\'s midpoint). Expenditure indexes are equal to expenditure ratios divided by GDP-weighted averages.'))
...
predictions <- predictions[type%in%sel_ord]
...
r2s=predictions[, list(#R2_diff = cor(dlnusales, dlnusales_hat, use='pairwise')^2,
                     ...
                     MAPE_lev = mean(abs((common_dv-common_dv_hat)/common_dv),na.rm=T),
                     N=.N),
 ...
 setkey(r2s, category, country, brand, type)
  ...
  print(kable(tmp2, format= 'html', caption =lbl, digits = 3) %>% kable_styling())
...
ord=c('ec_main', 
      #'ec_nommix', 'ec_onlyllen', 'ec_onlypr','ec_onlydst',
       #grep('unreXstricted',unique(predictions$type),value=T),
       #'ec_lntrend','ec_noendogeneity',
       'loglog_main',
      'loglog_noldv', 'loglog_onlyldv')



sel_ord = unique(unlist(sapply(paste0(ord,'$|', ord,'_holdout'), grep, x= unique(predictions$type),value=T,simplify=F)))

tmppred = predictions_kfold[type%in%sel_ord]

setorder(tmppred, type, category,country,brand,kfold,date)

#tmppred[grepl('log',type,ignore.case=T), dlnusales:=lnusales-lnlagusales]
tmppred[grepl('log',type,ignore.case=T), dlnusales_hat:=lnusales_hat-lnlagusales]


tmp = tmppred[, list(R2 = cor(dlnusales, dlnusales_hat, use='pairwise')^2,
                         MSE = mean((dlnusales-dlnusales_hat)^2,na.rm=T)),
                  by=c('type','category','country','brand','kfold','estim_set')]

# ADD TRAINING AND PREDICTION TO SET
# correct predictions

tmp2=tmp[type%in%sel_ord, lapply(.SD, mean), by = c('type', 'estim_set'), .SDcols=grep("R2|mse", colnames(tmp),value=T, ignore.case=T)]
...
lbl = 'Model selection (hold-out sample)'
 ...
 # setnames(tmp2, 'MSE_lev', 'MSE')
  ...
  print(kable(tmp4, format= 'html', caption =lbl, digits = 3) %>% add_header_above(list(' '=1, 'R-squared' =2, 'MSE'=2)))
...
r2=predictions_kfold[estim_set==F&type=='ec_main', list(r2=cor(dlnusales, dlnusales_hat, use='pairwise')^2), by = c('category','country','brand')]

setorder(r2, r2)


r2x=predictions_kfold[estim_set==F&type=='ec_main'& brand =='samsung' & category=='phones_smart'&country=='china']


r2x[,date:=as.Date(date)]

# selection
with(r2x, plot(date, lnlagusales + dlnusales_hat, lty=1,lwd=2,type='c', main = 'Predicted versus fitted values (in log levels)', xlab = 'Date',ylab='Log unit sales'))
with(r2x, lines(date, lnlagusales+dlnusales, lty=2, type = 'c'))
