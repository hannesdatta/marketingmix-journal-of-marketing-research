...
# correlatino power distance & individualism

countries <- unique(brand_panel, by = c('country'))
cor(countries$pdi, countries$idv)

elasticities$ec_main_sur[, list(cor(elastlt, elastmedianlt,use='pairwise')),by=c('variable')]
...
P. 24: For the point about δ12,1 that you make in the paper, could this be because larger countries may have longer product lines with more line length fluctuations; thus making it easier to estimate the line length elasticity? It sounds like an endogeneity argument for which you correct. 

Along the same lines, are there more price cuts among richer countries that may explain what you report for δ11,2? The finding that richer countries are more price sensitive is hard to wrap your head around (I do see your argument here, which is plausible). Please clarify.
...
correlation_matrix(data.frame(outc[,-1,with=F]))
Hmisc:::rcorr(as.matrix(outc[,-1,with=F]))

cor.test(outc$penn_gdppercap, outc$price_coefvar)



# Regression
if(0){
m1<-lm(llen~1+pop, data = tmp)
m2<-lm(llen_coefvar~1+pop, data = tmp)
m3<-lm(price~1+gdppercap, data = tmp)
m4<-lm(price_coefvar~1+gdppercap, data = tmp)

stargazer(m1, m2, m3, m4, type='text')
}
# w/ all controls
tmp2 <- merge(tmp, unique(elast, by = c('category','country','brand')), by = c('category','country','brand'), all.x=T, all.y=F)

 

 
m1<-lm(llen~1+ln_penn_popyravg_mc+ln_penn_percapitargdpeyravg_mc+ln_market_meangrowth_mc+sbbe_round1_mc+`brand_from_jp-us-ch-ge-sw`+ln_rwpspr_windex_mc+ln_wpswdst_windex_mc+ln_nov6sh_windex_mc+ln_market_herf_mc+appliance+ln_ginicoef_mc+ln_penn_growthrgdpeyravg_mc+ln_uai_mc+ln_pdi_mc+ln_mas_mc, data = tmp2)

m2<-lm(llen_coefvar~1+ln_penn_popyravg_mc+ln_penn_percapitargdpeyravg_mc+sbbe_round1_mc+`brand_from_jp-us-ch-ge-sw`+ln_rwpspr_windex_mc+ln_wpswdst_windex_mc+ln_llen_windex_mc+ln_nov6sh_windex_mc+ln_market_herf_mc+ln_market_meangrowth_mc+appliance+ln_ginicoef_mc+ln_penn_growthrgdpeyravg_mc+ln_uai_mc+ln_pdi_mc+ln_mas_mc, data = tmp2)

m3<-lm(price~1+ln_penn_percapitargdpeyravg_mc+sbbe_round1_mc+`brand_from_jp-us-ch-ge-sw`+ln_wpswdst_windex_mc+ln_llen_windex_mc+ln_nov6sh_windex_mc+ln_market_herf_mc+ln_market_meangrowth_mc+appliance+ln_penn_growthrgdpeyravg_mc+ln_ginicoef_mc+ln_penn_popyravg_mc+ln_uai_mc+ln_pdi_mc+ln_mas_mc, data = tmp2)

m4<-lm(price_coefvar~1+ln_penn_percapitargdpeyravg_mc+sbbe_round1_mc+`brand_from_jp-us-ch-ge-sw`+ln_rwpspr_windex_mc+ln_wpswdst_windex_mc+ln_llen_windex_mc+ln_nov6sh_windex_mc+ln_market_herf_mc+ln_market_meangrowth_mc+appliance+ln_penn_growthrgdpeyravg_mc+ln_ginicoef_mc+ln_penn_popyravg_mc+ln_uai_mc+ln_pdi_mc+ln_mas_mc, data = tmp2)
...
__Differences between short- and long-term elasticities__

```{r}

elast[, percst := (elast-elastlt)/elastlt]
# same sign?

elast[, same_sign := sign(elast)==sign(elastlt)]
table(elast$same_sign)

elast[same_sign==FALSE]

elast[, list(N=.N, elast = mean(elast), elastlt = mean(elastlt), percst = mean(percst)), by = c('variable')]
elast[, list(N=.N, elast = mean(elast), elastlt = mean(elastlt), percst = mean(percst), percst_1 = (1+mean(percst)))]
...
# after averaging: out$st$ftmelast/out$lt$ftmelast

elast[, list(cor(elast, elastlt)), by = c('variable')]
...
```{r correctsign, echo=F,results='asis'}

elast[, cor_z_val:=ifelse(grepl('pr',variable),-1,1)]
elast[,list(correct_sign=length(which(z_elastlt*cor_z_val>=zval))/.N,
           ns=length(which(abs(z_elastlt)<zval))/.N,
           incorrect_sign=length(which(z_elastlt*cor_z_val<=(-zval)))/.N)]
```
...
Verify mean price elasticities on only positive coefficients

```{r}
 tmp = elast[!is.na(elastlt)][, ':=' (val=elastlt, val_z=z_elastlt, val_w=w_elastlt)]
tmp[, theoreticalsign := 0]
tmp[elastlt<0&grepl('pr', variable), theoreticalsign:=1]
tmp[elastlt>0&!grepl('pr', variable), theoreticalsign:=1]

  tmp=tmp[theoreticalsign==1, list(Neffects=.N, 
   						  ftmelast = sum(val*val_w)/sum(val_w),
   						  median_elast = median(val), 
  						  interval90 = paste0('[',paste(sprintf("%.3f",quantile(val, c(.05,.95))),collapse=', '),']'),
  												  perc_positive = length(which(val_z>=(zval)))/.N, 
...
```
```{r}
# Correlation short & long-term

tmp = elast[!is.na(elastlt)][, ':=' (val=elastlt, val_z=z_elastlt, val_w=w_elastlt)]
tmpx=melt(elast[, c('category','country','brand','variable','elastlt','elast'),with=F])
setnames(tmpx, 'variable.1', 'type')
tmp2 = dcast(tmpx, category+country+brand~variable+type)

tmp[grepl('llen',variable), cor(elast, elastlt)]
tmp[grepl('pr',variable), cor(elast, elastlt)]
tmp[grepl('dst',variable), cor(elast, elastlt)]

# magntitude
tmp[, diff:=elast-elastlt]
tmp[, perc_change:= 100*diff/abs(elastlt)]

tmp[, list(mean(perc_change)),by=c('variable')]
...
```{r elasts_full_median, echo=F,results='asis'}
...
for (byvar in c('country', 'category', 'novar')) {
  tmp=tmp_elast[, list(N=.N,
                                  ...
                                  percpos = 100*length(which(abs(val/val_se)>=qval&val>0))/.N,
                                  percneg = 100*length(which(abs(val/val_se)>=qval&val<0))/.N,
                                  percns = 100*length(which(abs(val/val_se)<qval))/.N),
                                  by=c('variable', byvar)]
...
for (byvar in c('category')) {
  tmp=elast[!is.na(elastlt), list(N=.N,
                                  ...
                                  percpos = 100*length(which(abs(elastlt/elast_se)>=qval&elastlt>0))/.N,
                                  percneg = 100*length(which(abs(elastlt/elast_se)>=qval&elastlt<0))/.N,
                                  percns = 100*length(which(abs(elastlt/elast_se)<qval))/.N),
                                  by=c('variable', byvar)]
  tmp[, lbl := paste0(sprintf("%.3f",elast), ' ', sig)]
