...
# Load auxilary functions
  ...
  elast <- fread('../externals/elast_results_salesresponsenew.csv') 
...
covars_summary <- c('brandz', 'market_herf', 'market_growth', 'appliance', 'gini', 'local_to_market')
...
upd_models = list(m4 = .~1 + emerging + brandz + I(brandz*emerging)+ (1|country) + (1|category) + (1|brand) + ln_market_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market)
...
for (lev in c(.5,.25,.1)) {
 ...
 elast[brandz==0, l_brandequity:='weak brand', by = c('variable','category','country','market_id')]
 elast[brandz==1, l_brandequity:='strong brand', by = c('variable','category','country','market_id')]
