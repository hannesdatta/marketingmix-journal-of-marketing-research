...
tmp=unique(elast, by='country')[, c('country','penn_pop2010', 'penn_percapitargdpe2010','penn_growthrgdpe2010', 'ginicoef','pdi', 'uai','mas'),with=F]

setorderv(tmp, c('country'))

tmp=sanitize_table(tmp)

kable(tmp, format='html', digits=c(0,2, 0,2,2,2,2,2), format.args = list(big.mark = ",", format ='f'), initial.zero = FALSE, caption=tab('Development indicators for countries in sample^1^')) %>%
...
unitlist = list(
  ...
  list(
    ...
    units = c(
      ...
      'With Long-Run Competitive Effects' = 'ec_unrestrictedcompetition'
    ),
  ...
  list(
   ...
   list(
    units = c(
      'Elasticities calculated at the mean (main model)' = 'ec_main_sur',
      'Elasticities calculated at the median' = 'ec_at_median'
    ),
    title =  'Differences Mean/Median^1^'
  )
  #,
 # list(
  #  units = c(
...
lbl = 'Model selection (hold-out sample)'
 ...
 # setnames(tmp2, 'MSE_lev', 'MSE')
  ...
  print(kable(tmp4, format= 'html', caption =lbl, digits = 3) %>% add_header_above(list(' '=1, 'R-squared' =2, 'RMSE'=2, 'SMAPE'=2)))
