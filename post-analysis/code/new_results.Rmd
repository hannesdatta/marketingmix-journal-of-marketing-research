...
# Load auxilary functions
  ...
  elast=elast[!grepl('allothers|unbranded', brand,ignore.case=T)]
...
tmp=unique(elast, by='country')[, c('country','gdppercapitacurrent2010','gdpgrowth2010', 'ginicoef','developed'),with=F] #ci_overall_s','hdi2010', '')
...
kable(tmp, format='html', digits=c(2,0,2,2), format.args = list(big.mark = ","), initial.zero = FALSE, caption=tab('Development indicators for countries in sample^1^')) %>%
        kable_styling() %>% footnote(number=c('Countries ranked by GDP per capita.', "Sources to be updated.")) %>%
