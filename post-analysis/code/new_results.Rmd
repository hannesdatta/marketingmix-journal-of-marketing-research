...
tmp = dcast(elast,category+country+brand~variable, value.var=c('elastlt'))

#for (var in grep('elast', colnames(tmp),value=T)) {
#  tmp[, (paste0('mc_', var)):=get(var)-mean(get(var),na.rm=T), by = c('market_id')]
#  }

printtmp=tmp[, ordered_vars]#grep('elastlt[_]', colnames(tmp),value=T) ,with=F]
...
```{r withinR2, results='asis', echo = FALSE}
...
r2s=predictions[type%in%sel_ord, list(#R2_diff = cor(dlnusales, dlnusales_hat, use='pairwise')^2,
 ...
 setkey(r2s, category, country, brand, type)
  ...
  tmp = r2s[, lapply(.SD, mean,na.rm=T), by = c('type'), .SDcols=grep("(R2|MSE).*lev$",colnames(r2s),value=T)]
...
```{r holdoutrsq, results='asis', echo = FALSE}
...
```{r coefvar_by_cc2, echo=F,results='asis', include=TRUE}
