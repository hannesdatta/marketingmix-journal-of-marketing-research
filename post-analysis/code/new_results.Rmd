...
r2s=predictions[type%in%sel_ord, list(#R2_diff = cor(dlnusales, dlnusales_hat, use='pairwise')^2,
 ...
 setkey(r2s, category, country, brand, type)
  ...
  print(kable(tmp2, format= 'html', caption =lbl, digits = 3) %>% kable_styling()%>% footnote(number=paste0('R2 (MeAPE) is computed as the average squared correlation (average median percentage error) between predicted and observed sales, across the ', nrow(unique(elast, by =c('category','brand','country'))), ' estimated models.'))) 
