...
```{r elasticities_by_country, echo = FALSE, results='asis'}
  sigvalue = .1 # .1-level of significance
  ...
  for (i in 1:2) {
    ...
    tmp=tmp[, list(Neffects= .N, 
                   all_melast = sum(val*val_w)/sum(val_w),
						 		   all_z = sum(val_z,na.rm=T)/sqrt(.N)), by=c('variable', 'country')]

    tmp=dcast(tmp, country~variable,value.var='all_melast')
  out[[i]]<-tmp
  }
