...
p<-  ggplot(tmp[variable==.var], aes(x=group, y=qmeans, fill=ifelse(developed==1, 'developed', 'emerging'))) + 
  theme_bw() + 
...
for (.var in ordered_vars){
p<-ggplot(tmp[variable==.var], aes(x=group, y=qwmeans, fill=ifelse(developed==1, 'developed', 'emerging'))) + 
  theme_bw() + 
  geom_bar(position=position_dodge(), stat="identity") + 
  scale_fill_grey() + 
  labs(title = fig(paste0("Mean long-term ", rename.fkt(.var), " elasticities (weighted) per quartile"))) + 
  guides(fill=guide_legend("Development status")) +
  xlab("Quartiles") + 
  ylab("Elasticity")
print(p)
...
On sample of (`r length(unique(elast$brand))` unique brands.

## with standardized SBBE
...
## with SBBE (not standardized)

```{r randomcombined1bb, echo = FALSE, results='asis'}
m1 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + sbbe_mc
...
# interaction model
elast[, emerging:=1-developed]
m2 = ~1+ (1|brand) + (1|category) + (1|country) + emerging + appliance + I(appliance*emerging) + ln_market_growth_mc + I(ln_market_growth_mc*emerging) + ln_herf_mc + I(ln_herf_mc*emerging) + local_to_market + I(local_to_market*emerging) + western_brand + I(western_brand*emerging) + sbbe_mc + I(sbbe_mc*emerging)
...
## with log market share

```{r randomcombined1cc, echo = FALSE, results='asis'}
m1 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + ln_overall_ms_mc
...
# interaction model
elast[, emerging:=1-developed]
m2 = ~1+ (1|brand) + (1|category) + (1|country) + emerging + appliance + I(appliance*emerging) + ln_market_growth_mc + I(ln_market_growth_mc*emerging) + ln_herf_mc + I(ln_herf_mc*emerging) + local_to_market + I(local_to_market*emerging) + western_brand + I(western_brand*emerging) + ln_overall_ms_mc + I(ln_overall_ms_mc*emerging)
...
## continuing with standardized SBBE
