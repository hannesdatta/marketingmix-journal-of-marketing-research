...
```{correl, echo=FALSE,results='asis'}
...
print(corstars(printtmp, method=c("pearson"),result='html'))
...
Price index | Brand's average price (deflated sales value devided by the brand's sales volume over the observation period), indexed by the maximum price in a given category and country.
...
## Assessing interactions individually for short- and long-term elasticities
...
for (elasttype in c('st', 'lt')) {
  ...
  res=lapply(out1, function(m) lapply(ifelse(elasttype=='st', m$st, m$lt), function(n) data.table(mmix=m$variable,variable=rownames(summary(n)$coefficients),summary(n)$coefficients)))
  ...
  # MAIN MODEL
  cat("<P style='page-break-before: always'>")
  printout(mod_out, elasttype, title = tab(paste('Regression with ', ifelse(elasttype=='st', 'short-term', 'long-term'), ' elasticities in a pooled sample of developed and emerging countries: testing for interactions')), vars=ordered_vars, notes=paste0(ifelse(elasttype=='st', gsub('long[-]term', 'short-term', notes_base), notes_base),  ' For each variable, the first column reports a model with full interactions; the second column reports a model in which only interactions are added that were significant in an auxilary regression with all main effects and only the relevant interaction.'))
}
...
Currently, we use two types of rolling windows:
(a) prices are sales-weighted (months t-2, t-1, t) averages of a brand's SKUs prices. 
(b) new product activity is defined as the share of SKUs introduced in the past six months (months t-5, t-4, ... t)

We should discuss whether it makes sense re-estimating all models with either both variables using (a) t-2, t-1, t, or (b) t-5, t-4, ... t.

## Alternative measures for a country's state of development

- Emerging country indicator (here reported as 1-emerging = developed country indicator)
...
## Alternative measures for Brand Equity
...
Options are:
(a) removing outliers (e.g., +/- 3x SD of elasticities)
(b) Winsorized elasticities (one or two-tailed? 5%?)

## Assessing time-varying elasticities

Time-varying elasticities could be assessed by splitting the sample. Options are:
- 1/3 vs 2/3
- first versus last 50%

We would then estimate marketing mix elasticities in twosamples, and assess the extent to which they (systematically) differ. 

Drawback is that we'd lose some categories, as a split won't yield enough observations for estimation. A way out would be to split the sample in the first 70% of observations, and the last 70% of observations (hence, we'd have an overlap in both samples). We'd still lose some categories, but we should be better off compared to a hard split.
...
kable(tmp, caption = tab('Country-of-origins for brands in sample, ordered by the number of brands from a given country^1^', prefix ='A'),format='html') %>%
        kable_styling() %>% footnote(number=paste0('Brand names listed alphabetically.'))
...
for (byvar in c('category', 'country')) {
  ...
  cat("<P style='page-break-before: always'>")
  if(0){print(kable(tmp, digits=3, caption = tab(paste0('Long-term elasticities (by ', byvar, ')^1^'), prefix='A'), format='html', initial.zero = FALSE) %>%
