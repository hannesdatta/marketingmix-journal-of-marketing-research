...
m1 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + othercountry + sbbe_std_mc

m2 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + othercountry + sbbe_mc

m3 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + othercountry + ln_overall_ms_mc

m4 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + othercountry + ln_gci_overall_s

m5 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + othercountry + ln_gci_sub_basicrequire_s + ln_gci_sub_efficiencyenhance_s + ln_gci_sub_innovation_s
...
m1 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + othercountry + sbbe_std_mc
m1b = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + sbbe_std_mc
...
cat("<P style='page-break-before: always'>")
out2=regmodel(formula=m1b, dat=elast[developed==1], model='lmer')
covlabels=unlist(sanitize_table(data.frame(colnames(out2[[1]]$lt[[1]]@pp$X))))
covlabels=c(covlabels[-which(covlabels=='(Intercept)')], 'Constant')
...
m2 = ~1+ (1|brand) + (1|category) + (1|country) + emerging + appliance + I(appliance*emerging) + ln_market_growth_mc + I(ln_market_growth_mc*emerging) + ln_herf_mc + I(ln_herf_mc*emerging) + local_to_market + I(local_to_market*emerging) + western_brand + I(western_brand*emerging) + sbbe_std_mc + I(sbbe_std_mc*emerging) + othercountry
...
m1 = ~1+ (1|brand) + (1|category) + as.factor(country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + othercountry + sbbe_std_mc
...
tmp = elast[, list(.N), by = c('brand', 'country_of_origin', 'western_brand', 'asian_country')]
tmp = tmp[, list(nbrands=length(unique(brand)), all_brands=paste(stri_trans_totitle(brand), collapse=', ')), by = c('country_of_origin', 'western_brand', 'asian_country')]
...
tmp[, other_country:=ifelse(western_brand==FALSE&asian_country==0, 'yes', NA)]
tmp[, western_brand:=as.character(ifelse(western_brand==T, 'yes', NA))]
tmp[, asian_country:=as.character(ifelse(asian_country==1, 'yes', NA))]
setorderv(tmp, 'nbrands', order=-1L)
setcolorder(tmp, c('country_of_origin', 'nbrands', 'western_brand', 'asian_country', 'other_country', 'all_brands'))

tmp=sanitize_table(tmp)

kable(tmp, caption = tab('Country-of-origins for brands in sample, ordered by the number of brands from a given country', prefix ='A'))
```
...
m1 = ~1+ appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + othercountry + sbbe_std_mc
