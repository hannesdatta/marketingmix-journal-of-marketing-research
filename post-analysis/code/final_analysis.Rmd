...
covlabels=unlist(sanitize_table(data.frame(unique(unlist(lapply(onlybe[[1]]$lt, function(x) colnames(x@pp$X)))))))
covlabels=c(covlabels[-which(covlabels=='(Intercept)')], 'Constant')

cat("<P style='page-break-before: always'>")
printout(onlybe, 'stlt', omit=NULL, title = tab('Regression with short- and long-term elasticities in a pooled sample of developed and emerging countries, only with brand equity interactions'), vars=ordered_vars, notes=notes_base, covlabels=covlabels, column.separate=rep(2, each=length(out1)),
         dep.var.labels.include=T, dep.var.labels=rep(c('short-term', 'long-term'),4),
         table.layout=c('-c#dm-t-a-n'))
...
cat("<P style='page-break-before: always'>")
printout(out1, 'lt', omit=NULL, title = tab('Regression with long-term elasticities on covariates and different indicators for a country\'s state of development'), vars=varsord[3:4], notes=notes_base, covlabels=covlabels, column.separate=unlist(lapply(out1[varsord[3:4]], function(x) length(x$lt))))
sink()
