...
Brand equity               | Sales-based brand equity, obtained as a transformation of the brand-specific intercept from the market share attraction model (short-term, long-term, and standardized by category)    | Datta, Ailawadi, and van Heerde (2017)
Brand-level market share  | Log of average market share of brand in the observation period
...
out=lapply(covars, function(.var) {
  ...
  tmp[, variable_label:=gsub(' activity', ' a.', as.factor(rename.fkt(variable)))]
  return(tmp)
})
...
tmp[, splitlabel := paste0(rename.fkt(splitname), ': ', splitvar)]
...
printout(out1[1:2], 'lt', omit=NULL, title = tab('Model selection: regression with long-term elasticities in a pooled sample of developed and emerging countries'), vars=varsord[1:2], notes=notes_base, covlabels=covlabels, column.separate=unlist(lapply(out1[1:2], function(x) length(x$lt))))

cat("<P style='page-break-before: always'>")
printout(out1[3:4], 'lt', omit=NULL, title = tab('Model selection: regression with long-term elasticities in a pooled sample of developed and emerging countries'), vars=varsord[3:4], notes=notes_base, covlabels=covlabels, column.separate=unlist(lapply(out1[3:4], function(x) length(x$lt))))
