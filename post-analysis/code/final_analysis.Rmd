...
form_std = .~. + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand
...
Brand equity               | Sales-based brand equity, obtained as a transformation of the brand-specific intercept from the market share attraction model (can be standardized by category)    | Datta, Ailawadi, and van Heerde (2017)
...
covars <- c('appliance', 'market_growth', 'herf', 'local_to_market', 'western_brand', 'sbbe_std', 'sbbe', 'overall_ms', 'overall_prindex', 'brandz', 'gci_overall_s', 'gci_sub_basicrequire_s', 'gci_sub_efficiencyenhance_s', 'gci_sub_innovation_s')
...
modellist = lapply(c('sbbe_std_mc', 'sbbe_mc', 'ln_overall_ms_mc', 'ln_overall_prindex_mc', 'brandz'), 
...
printout(out1, 'lt', omit=NULL, title = tab('Model selection: regression with long-term elasticities in a pooled sample of developed and emerging countries'), vars=varsord, notes=notes_base, covlabels=covlabels, column.separate=unlist(lapply(out1, function(x) length(x$lt))))
