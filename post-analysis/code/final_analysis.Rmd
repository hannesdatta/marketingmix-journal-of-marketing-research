...
tmp = elast[!is.na(get(obj[1]))][, ':=' (val=get(obj[1]), 
    ...
    # random effects based p values
        diff_tests=rbindlist(lapply(unique(tmp$variable), function(.var) {
          print(.var)
        data.table(variable=.var, effect=c('constant', 'emerging'), summary(lmer(val~1+(1|category) + (1|country) + (1|brand) + emerging, weights=val_w,REML=FALSE,data=tmp[variable==.var]))$coefficients)[, p:=2*(1-pnorm(abs(get('t value'))))]
      }))
   
 obj=c('elastlt','w_elastlt', 'z_elastlt')
       
obj=c('elastlt','w_elastlt', 'z_elastlt')
summary(lmer(val~1+(1|category) + (1|country) + (1|brand) + emerging, weights=val_w,data= tmp[variable=='llen'],control = lmerControl(optimizer ="Nelder_Mead")))
...
tmp = elast[!is.na(get(obj[1]))][, ':=' (val=get(obj[1]), 
                                       val_w=get(obj[2]),
                                       val_z=get(obj[3]))]
summary(lmer(val~1+(1|category) + (1|country) + emerging, data= tmp[variable=='rwpspr']))
