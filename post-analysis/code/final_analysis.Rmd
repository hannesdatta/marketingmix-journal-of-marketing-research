...
pl<-lapply(c(qmeans='qmeans',qwmeans='qwmeans'), function(varplot) {
...
------------------------- | --------------------------------------------------------|----------
Appliance (versus electronics)| Dummy, equaling 1 for appliances (washing machines, microwaves and refridgerators), 0 if not.
Category growth       | Log of a market's average growth rate in the observation period (geometric average)
...
Brand-level market share  | Average market share of brand in the observation period
Price index | A brand's average price (deflated sales value devided by the brand's sales volume over the observation period), indexed by the maximum price in a given category and country
Global competitiveness index (overall) | ...
Subindex basic requirements | ...
Subindex efficiency enhancers | ...
Subindex innovation and sophistication factors | ...
...
covars <- c('appliance', 'market_growth', 'herf', 'local_to_market', 'western_brand', 'sbbe_std', 'sbbe', 'overall_ms', 'overall_prindex', 'gci_overall_s', 'gci_sub_basicrequire_s', 'gci_sub_efficiencyenhance_s', 'gci_sub_innovation_s')
...
kable(dtf, digits=3,caption = tab(paste0('Summary statistics for covariates in second-stage analysis')),
...
#elast=elast[globalbrand==T&!brand%in%c('unbranded')]

```

On sample of `r length(unique(elast$brand))` unique brands (excluding one 'unbranded' brand).

## Model selection 

```{r modelselection, echo = FALSE, results='asis'}
m1 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + sbbe_std_mc

m2 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + sbbe_mc

m3 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + ln_overall_ms_mc

m4 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + ln_overall_prindex_mc

#m4 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + ln_gci_overall_s

#m5 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + ln_gci_sub_basicrequire_s + ln_gci_sub_efficiencyenhance_s + ln_gci_sub_innovation_s

out1=regmodel(formula=c(m1,m2,m3, m4), dat=elast, model='lmer')
...
printout(out1, 'lt', omit=NULL, title = tab('Model selection: regression with long-term elasticities in a pooled sample of developed and emerging countries'), vars=varsord, notes=notes_base, covlabels=covlabels, column.separate=unlist(lapply(out1, length)))
...
m1 = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + sbbe_std_mc
m1b = ~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + sbbe_std_mc + emerging

names(ordered_vars) <- paste0(unlist(sanitize_table(data.frame(ordered_vars))), ' elasticity')

out1=regmodel(formula=list(m1,m1b), dat=elast, model='lmer')
...
printout(out1, 'st', omit=NULL, title = tab('DV: short-term elasticities, pooled sample of developed and emerging countries'), vars=ordered_vars,notes=gsub('long[-]term', 'short-term', notes_base), covlabels=covlabels,
         column.separate=unlist(lapply(out1, length)))
...
# interaction model
m2 = ~1+ (1|brand) + (1|category) + (1|country) + emerging + appliance + I(appliance*emerging) + ln_market_growth_mc + I(ln_market_growth_mc*emerging) + ln_herf_mc + I(ln_herf_mc*emerging) + local_to_market + I(local_to_market*emerging) + western_brand + I(western_brand*emerging) + sbbe_std_mc + I(sbbe_std_mc*emerging)
...
m1 = ~1+ (1|brand) + (1|category) + as.factor(country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + sbbe_std_mc
...
tmp = elast[, list(.N), by = c('brand', 'country_of_origin', 'western_brand', 'asian_brand', 'other_brand')]
tmp = tmp[, list(nbrands=length(unique(brand)), all_brands=paste(stri_trans_totitle(brand[order(brand)]), collapse=', ')), by = c('country_of_origin', 'western_brand', 'asian_brand', 'other_brand')]
...
setcolorder(tmp, c('country_of_origin', 'nbrands', 'western_brand', 'asian_brand', 'other_brand', 'all_brands'))
...
```{r elast_by_category_win, echo=F,results='asis'}
  tmp=elast[!is.na(elastlt), list(elast = mean(winsor(elastlt, trim=.01))), by=c('variable', 'category')]
  ...
  setorder(tmp, Category)
  cat("<P style='page-break-before: always'>")
  kable(tmp, digits=3, caption = tab('Long-term elasticities (by category, winsorized)', prefix='A'), format='html', initial.zero = FALSE) %>%
        kable_styling() %>% footnote(general=paste0('Note: Table reports winsorized long-term elasticities. Categories ordered alphabetically.'))
  
```	

```{r elast_by_country_win, echo=F,results='asis'}
  tmp=elast[!is.na(elastlt), list(elast = mean(winsor(elastlt, trim=.01))), by=c('variable', 'country')]
  ...
  setorder(tmp, Country)
  cat("<P style='page-break-before: always'>")
  kable(tmp, digits=3, caption = tab('Long-term elasticities (by country, winsorized)', prefix='A'), format='html', initial.zero = FALSE) %>%
        kable_styling() %>% footnote(general=paste0('Note: Table reports winsorized long-term elasticities. Countries ordered alphabetically.'))
...
m1 = ~1+ appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + sbbe_std_mc

interact1 = . ~ . + appliance*ln_gdppercap2010_mc + ln_market_growth_mc*ln_gdppercap2010_mc +
...
cat("<P style='page-break-before: always'>")
  ...
  out = lapply(list(st=c('elast', 'elast_se'), lt = c('elastlt','elastlt_se')), function(obj) {
    ...
    tmp=tmp[, list(Neffects=.N, median_elast = median(val), 
												  ...
												  win1_elast = mean(winsor(val, trim=.01)),
												  win5_elast = mean(winsor(val, trim=.05)),
...
for (iter in iters) {
  print(kable(sanitize_table(out[[iter[1]]]), digits=3, format='html', 
        ...
        footnote(general=paste0('Note: Table reports ', tolower(iter[2]), ' elasticities. Weights, if used, are equal to inverse standard errors of estimated elasticities. Significance tested at p<',sigvalue, '.')))
}
