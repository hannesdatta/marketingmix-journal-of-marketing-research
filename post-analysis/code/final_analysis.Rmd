...
```{r elast_plot2, echo=F}
ggarrange(plotlist=pl$qwmeans, common.legend=TRUE, legend='bottom')
```
...
Brand-level market share  | Average market share of brand in 2010
...
covars <- c('appliance', 'market_growth', 'herf', 'local_to_market', 'western_brand', 'other_brand', 'sbbe_std', 'sbbe', 'overall_ms', 'gci_overall_s', 'gci_sub_basicrequire_s', 'gci_sub_efficiencyenhance_s', 'gci_sub_innovation_s')

tmp=data.table(elast)
tmp = unique(tmp, by = c('market_id','brand'))
tmp = tmp[, covars, with=F]

# to b de done.
dtf = t(sapply(tmp, each(min,max,mean, sd)))
...
out1=regmodel(formula=c(m1,m2,m3, m4, m5), dat=elast, model='lmer')
...
printout(out1, 'lt', omit=NULL, title = tab('Model selection: regression with long-term elasticities in a pooled sample of developed and emerging countries'), vars=varsord, notes=notes_base, covlabels=covlabels, column.separate=c(5,5,5,5))
...
covlabels=unlist(sanitize_table(data.frame(unique(unlist(lapply(out2[[1]]$lt, function(x) colnames(x@pp$X)))))))
covlabels=c(covlabels[-which(covlabels=='(Intercept)')], 'Constant')
...
m1 = ~1+ appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + western_brand + other_brand + sbbe_std_mc
