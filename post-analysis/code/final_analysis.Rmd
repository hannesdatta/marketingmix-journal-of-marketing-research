...
notes_sig = '\*\* test &ast; *p*<.1, &ast;&ast; *p*<.05, &ast;&ast;&ast; *p*<.01 (two-sided).'
...
```{r elasticities_overview, echo = FALSE, results='asis'}
  ...
  out = lapply(list(st=c('elast', 'w_elast', 'z_elast'), lt = c('elastlt','w_elastlt', 'z_elastlt')), function(obj) {
    ...
    tmp = merge(tmp, diff_tests, by = c('variable'))
                   ...
                   all_melast = sum(val*val_w)/sum(val_w),
				           ...
				           em_melast = sum(val[developed==0]*val_w[developed==0])/sum(val_w[developed==0]),
									 ...
									 em_sig = signstars(sum(val_z[developed==0])/sqrt(length(which(!is.na(val[developed==0]))))),
									                    comb_t=unique(diff_t), comb_sig=psignstars(unique(diff_p))
									                    ), by=c('variable')]
...
Category concentration     | Log of Herfindahl index in a market (category/country combination).
...
*Note:* All continuous variables are first logged and then mean-centered before model estimation (except brand equity, which is only mean-centered as this variable can take on negative values). 

^1^ Geometric growth rate in a market (category/country combination), defined as the n-th's square root of the ratio of the total number of unit sales in a market in the last year over total number of unit sales in the first year (both years having 12 months of data). N equals to the number of years in the observation window.
...
kable(dtf, digits=3,caption = tab(paste0('Summary statistics for covariates in second-stage analysis, split by a country\'s development status')),
      format='html', initial.zero = FALSE) %>%
        kable_styling() %>% footnote(general=paste0('Note: Summary statistics for variables in levels, computed across ', nobs, ' observations in our sample.')) %>%
...
out1=regmodel(formula=c(main=form_main_model, 
                        interact=form_interact, 
                        onlybe=update.formula(form_main_model, .~.+I(sbbe_std_mc * emerging))), dat=elast, model='lmer')
...
onlybe = lapply(out1, function(x) {x$lt <- list(x$lt$onlybe); x$st <- list(x$st$onlybe); x})
...
# ONLY BE INTERACTIONS
cat("<P style='page-break-before: always'>")
printout(onlybe, 'stlt', omit=NULL, title = tab('Regression with short- and long-term elasticities in a pooled sample of developed and emerging countries, only with brand equity interactions'), vars=ordered_vars, notes=gsub('long[-]term', 'short- and long-term', notes_base))
  
# ALL INTERACTIONS
#cat("<P style='page-break-before: always'>")
#printout(interact, 'stlt', omit=NULL, title = tab('Regression with short- and long-term elasticities in a pooled sample of developed and emerging countries, with interactions'), vars=ordered_vars, notes=notes_base)       

#sink()

```

## Assessing interactions individually

### Short-term

```{r main_regs_st, echo = FALSE, results='asis'}
...
# assess significance
res=lapply(out1, function(m) lapply(m$st, function(n) data.table(mmix=m$variable,variable=rownames(summary(n)$coefficients),summary(n)$coefficients)))
...
summary_sig=dcast(res, variable ~ mmix, value.var='z')
summary_sig[,variable:=gsub('I[(]', '', variable)]
summary_sig[,variable:=gsub(' [*] emerging[)]', '', variable)]

              
kable(sanitize_table(summary_sig), format='html', initial.zero = FALSE, digits=3, caption=tab('Z-values for adding interactions to a main-effects only model for short-term elasticities')) %>%
        kable_styling()

update_forms=res[abs_z>1.65, list(interactions=unique(variable)),by=c('mmix')]

run_interact=sapply(ordered_vars, function(x) {
...
out1=regmodel(formula=c(form_interact, run_interact), dat=elast, model='lmer')

# reassemble to keep only the ones with terms we selected
mod_out<-lapply(seq(along=out1), function(i) {
  x=out1[[i]]
  x$lt<-list(x$lt[[1]], x$lt[[1+i]])
  x$st<-list(x$st[[1]], x$st[[1+i]])
  return(x)
})
names(mod_out) <- names(out1)
...
# MAIN MODEL
cat("<P style='page-break-before: always'>")
printout(mod_out, 'st', title = tab('Regression with short-term elasticities in a pooled sample of developed and emerging countries: testing for interactions'), vars=ordered_vars, notes=paste0(gsub('long[-]term', 'short-term', notes_base),  ' For each variable, the first column reports a model with full interactions; the second column reports a model in which only interactions are added that were significant in an auxilary regression with all main effects and only the relevant interaction.'))


```
### Long-term

```{r main_regs_lt, echo = FALSE, results='asis'}
...
# assess significance
res=lapply(out1, function(m) lapply(m$lt, function(n) data.table(mmix=m$variable,variable=rownames(summary(n)$coefficients),summary(n)$coefficients)))
...
kable(sanitize_table(summary_sig), format='html', initial.zero = FALSE, digits=3, caption=tab('Z-values for adding interactions to a main-effects only model for long-term elasticities')) %>%
        kable_styling()
...
# MAIN MODEL
cat("<P style='page-break-before: always'>")
printout(mod_out, 'lt', title = tab('Regression with long-term elasticities in a pooled sample of developed and emerging countries: testing for interactions'), vars=ordered_vars, notes=paste0(notes_base,  ' For each variable, the first column reports a model with full interactions; the second column reports a model in which only interactions are added that were significant in an auxilary regression with all main effects and only the relevant interaction.'))
...
- Developing country indicator (1-emerging country indicator)
...
cat("<P style='page-break-before: always'>")
printout(out1, 'lt', vars=ordered_vars[1:2], title = tab('Regression with long-term elasticities on covariates and different indicators for a country\'s state of development'),  notes=notes_base)

cat("<P style='page-break-before: always'>")
printout(out1, 'lt', vars=ordered_vars[3:4], title = tab('Regression with long-term elasticities on covariates and different indicators for a country\'s state of development'),  notes=notes_base)
#sink()
...
- Brand equity (SBBE)
- Log of brand-level market share
- BrandZ dummy variable, equaling 1 if a brand is listed in either *BrandZ's Top 100 Most Valuable global brands (2010)*, or *BrandZ's Top 50 Most Valuable Chinese Brands (2011)*.
...
cat("<P style='page-break-before: always'>")
printout(out1, 'lt', title = tab('Regression with long-term elasticities on covariates and different indicators for brand equity'), vars=ordered_vars[1:2], notes=notes_base)

cat("<P style='page-break-before: always'>")
printout(out1, 'lt', omit=NULL, title = tab('Regression with long-term elasticities on covariates and different indicators for brand equity'), vars=ordered_vars[3:4], notes=notes_base)
#sink()
```
...
for (byvar in c('category', 'country')) {
  ...
  cat("<P style='page-break-before: always'>")
  if(0){print(kable(tmp, digits=3, caption = tab(paste0('Long-term elasticities (by ', byvar, ')'), prefix='A'), format='html', initial.zero = FALSE) %>%
...
m1 = ~1+ appliance + ln_market_growth_mc + ln_herf_mc + local_to_market  + ln_overall_prindex_mc + sbbe_std_mc
...
- Number of markets (category/country combinations): `r length(unique(elast_all$market_id))`.
...
#print(xtable(tabl, caption = tab('Overview of categories and countries, with number of brands indicated for each market (category/country combination) in the estimation data')), scalebox='0.6',type='html',caption.placement='top')
tabl=sanitize_table(tabl)
setorder(tabl, Category)
kable(tabl, caption = tab('Overview of categories and countries (with number of brands indicated for each category/country combination), in estimation data', prefix='A'))
