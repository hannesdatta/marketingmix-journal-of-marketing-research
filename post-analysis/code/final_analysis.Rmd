...
printout(out1[1:2], 'lt', omit=NULL, title = tab('Model selection: regression with long-term elasticities in a pooled sample of developed and emerging countries'), vars=varsord, notes=notes_base, covlabels=covlabels, column.separate=unlist(lapply(out1[1:2], function(x) length(x$lt))))

cat("<P style='page-break-before: always'>")
printout(out1[3:4], 'lt', omit=NULL, title = tab('Model selection: regression with long-term elasticities in a pooled sample of developed and emerging countries'), vars=varsord, notes=notes_base, covlabels=covlabels, column.separate=unlist(lapply(out1[3:4], function(x) length(x$lt))))
