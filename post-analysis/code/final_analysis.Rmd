...
notes_loglik = 'Lok-likelihood ratio tests computed relative to preceding models.'
...
covars = c("(Intercept)", 
           ...
           "ln_herf_mc", "ln_market_growth_mc", "appliance", "ln_gini_mc", "local_to_market", "mean_ms_mc")
...
annotate_figure(p, top=fig('Elasticity plots'), bottom = text_grob('Note: Plots show estimated long-term marketing-mix elasticities (weighted by inverse standard errors), based on a country\'s degree of development (DM versus EM), and a median split on brand equity.', size= 8, just='left', x = 0))
```
...
notes_new = paste0(notes_base, ' Lok-likelihood ratio tests computed relative to preceding models. Model includes lagged values of a brand\'s marketing mix (instead of a brand\'s lagged market share) to allow for differential carry-over effects for each marketing-mix instrument.')
