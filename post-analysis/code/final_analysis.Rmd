...
kable(sanitize_table(tmp), format='html', format.args = list(big.mark = ","), initial.zero = FALSE, caption=tab('Development indicators for countries in sample^1^')) %>%
...
```{r elasts_full, echo=F,results='asis'}
  ...
  out = lapply(list(st=c('elast', 'z_elast', 'w_elast'), lt = c('elastlt','z_elastlt','w_elastlt')), function(obj) {
    ...
    tmp=tmp[, list(Neffects=.N, median_elast = median(val), 
												  ...
												  interval90 = paste0('[',paste(formatC(quantile(val, c(.05,.95))),collapse=', '),']')),
...
for (iter in iters) {
  ...
  print(kable(sanitize_table(out[[iter[1]]]), digits=3, format='html', format.args = list(big.mark = ","),
        ...
        footnote(number=c(paste0('Table reports ', tolower(iter[2]), ' elasticities. Significance tested at *p*<',sigvalue, '.', ' The number of observations differs slightly across marketing mix instruments because some brands in some markets (category/country combinations) do not have variation in these variables.'), estimnote)))
...
for (i in 1:4) {
  ...
  printout(outx, 'lt', title = tab(paste0('Robustness check with long-term ', gsub('ity', 'ities', names(ordered_vars[i])), ' and alternative metrics for a country\'s development degree'), prefix='A'), vars=ordered_vars[i],  notes=paste0(gsub('long[-]term', 'short- and long-term', notes_new), ''), covariate_choices = covars)
}
sink()
