...
elast[, ':=' (highbe=sbbe>median(sbbe), brandeq_label=ifelse(sbbe>median(sbbe), 'High Brand Equity','Low Brand Equity')), by = c('market_id')]
...
tmp=tmp[, list(Neffects= .N, 
                 melast = sum(val*val_w)/sum(val_w),
						 		 z = sum(val_z,na.rm=T)/sqrt(.N)), by = c('variable', 'country_class','highbe', 'brandeq_label')]

# add N

for (var in ordered_vars) {
  dtf=dcast(tmp[variable==var], country_class~brandeq_label, value.var='melast')
  nobs=sum(tmp[variable==var]$Neffects)
  colnames(dtf)[1]='EMPTY'
print(kable(sanitize_table(dtf), format='html', initial.zero = FALSE, digits=3,caption = tab(paste0('Weighted mean ', gsub('ity', 'ities', names(var)), ' by brand equity and a country\'s development status^1^'))
      ) %>% kable_styling() %>% footnote(number=c(paste0('Computed across ', nobs, ' observations in our sample.'))))#,'Test on differences carried out by regressing variables on an intercept and an emerging country indicator variable. Reported t-value is obtained from the estimate of the emerging country indicator.')) %>% add_header_above(c(" " = 1, "Emerging countries" = 4, "Developed countries" = 4, "Test on differences^2^"=2)))
...
notes_new = "^1^ Elasticities are weighted by inverse standard errors. Standard errors in parentheses. Continuous variables have been mean-centered; constant to be interpreted as (weighted) mean elasticity in developed markets."

sink('test.htm')


printout(outx, 'lt', title = tab('Regression with long-term elasticities'), vars=ordered_vars,  notes=gsub('long[-]term', 'short- and long-term', notes_new), covariate_choices = covars)
sink()
