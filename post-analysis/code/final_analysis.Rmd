...
out1=regmodel(formula=c(upd_models), dat=elast_nov, model='lmer')

cat("<P style='page-break-before: always'>")

printout(out1, 'lt', title = tab(paste0('Robustness check with new-product activity: Regression with long-term elasticities'), prefix='A'), vars=c(ordered_vars, 'New-product activity elasticity'=grep('^nov', unique(elast_nov$variable),value=T)),  notes=notes_base, covariate_choices = covars)

```

```{r variable_means_for_plots, echo=FALSE, results='asis'}
...
tmp = dcast(sims,variable+l_brandequity~l_emerging,value.var='est')
setnames(tmp, 'l_brandequity', 'sbbe_std')

print(kable(sanitize_table(tmp), format='html', initial.zero = FALSE, digits=3,caption = tab(paste0('Weighted elasticities by brand equity and a country\'s development degree'), prefix='A')) %>% kable_styling())
