...
form_main_model = .~1+ (1|brand) + (1|category) + (1|country) + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + ln_overall_prindex_mc + sbbe_std_mc + emerging
...
out1=regmodel(formula=form_main_model, dat=elast, model='lmer')
...
printout(out1, 'stlt', omit=NULL, title = tab('Model selection: regression with long-term elasticities in a pooled sample of developed and emerging countries'), vars=ordered_vars, notes=notes_base, covlabels=covlabels, column.separate=rep(2, each=length(out1)),
