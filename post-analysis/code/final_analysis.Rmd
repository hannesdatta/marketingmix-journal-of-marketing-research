...
```{r 2by2, echo = FALSE, results = 'asis'}

elast[, ':=' (highbe=sbbe>median(sbbe), highbe_label=ifelse(sbbe>median(sbbe), 'High Brand Equity','Low Brand Equity')), by = c('market_id')]
elast[, ':=' (brandz_label=ifelse(brandz==1, 'High Brand Equity','Low Brand Equity')), by = c('market_id')]
elast[, ':=' (brandz_globalonly_label=ifelse(brandz_globalonly==1, 'High Brand Equity','Low Brand Equity')), by = c('market_id')]
elast[, ':=' (brandz_chinaonly_label=ifelse(brandz_chinaonly==1, 'High Brand Equity','Low Brand Equity')), by = c('market_id')]

elast[, ':=' (brandz_financial500_label=ifelse(brandz_financial500==1, 'High Brand Equity','Low Brand Equity')), by = c('market_id')]
elast[, ':=' (brandz_global_alltime_label=ifelse(brandz_global_alltime==1, 'High Brand Equity','Low Brand Equity')), by = c('market_id')]

  
# use long-term elasticities
obj=c('elastlt','w_elastlt', 'z_elastlt')

for (it in c('sbbe', 'brandz100','brandz', 'brandzchina', 'brandz_all', 'brandz500')) {
  tmp = elast[!is.na(get(obj[1]))][, ':=' (val=get(obj[1]), 
                                       val_w=get(obj[2]),
                                       val_z=get(obj[3]))]

  if (it == 'sbbe') {
    key = c('highbe', 'highbe_label')
    tablelabel = '(SBBE definition)'
    tmp[, label:=get(rev(key)[1])]
    form=val~country_class*label
   }
  if (it == 'brandz') {
    key = c('brandz', 'brandz_label')
    tablelabel = '(BrandZ definition: global 100 & china 50)'
    tmp[, label:=get(rev(key)[1])]
    form=val~country_class*label
   }
  if (it == 'brandz100') {
    key = c('brandz_globalonly', 'brandz_globalonly_label')
    tablelabel = '(BrandZ definition: global 100)'
    tmp[, label:=get(rev(key)[1])]
    form=val~country_class*label
   }
  if (it == 'brandzchina') {
    key = c('brandz_chinaonly', 'brandz_chinaonly_label')
    tablelabel = '(BrandZ definition: China 50 - estimated only in Chinese)'
    tmp[, label:=get(rev(key)[1])]
    tmp=tmp[country=='china']
    form=val~label
   }
  if (it == 'brandz500') {
    key = c('brandz_financial500', 'brandz_financial500_label')
    tablelabel = '(Brand Finance 500)'
    tmp[, label:=get(rev(key)[1])]
    form=val~country_class*label
   }
  if (it == 'brandz_all') {
    key = c('brandz_global_alltime', 'brandz_global_alltime_label')
    tablelabel = '(BrandZ: Top 100 2006-2013)'
    tmp[, label:=get(rev(key)[1])]
    form=val~country_class*label
   }
  
  
  tmpdat=tmp[, list(Neffects= .N, 
                 melast = sum(val*val_w)/sum(val_w),
						 		 z = sum(val_z,na.rm=T)/sqrt(.N)), by = c('variable', 'country_class', key, 'label')]

for (var in ordered_vars) {
  maov <- aov(form, data= tmp[variable==var], weights=val_w)
  
  dtf=dcast(tmpdat[variable==var], country_class~label, value.var='melast')
  nobs=sum(tmpdat[variable==var]$Neffects)
  colnames(dtf)[1]='EMPTY'
  
print(kable(sanitize_table(dtf), format='html', initial.zero = FALSE, digits=3,caption = tab(paste0('Mean ', gsub('asticity', 'asticies', names(ordered_vars)[match(var, ordered_vars)]), ' by brand equity and a country\'s development status ', tablelabel, '^1^'))
      ) %>% kable_styling() %>% footnote(number=c(paste0('Computed across ', nobs, ' observations in our sample. ', estimnote))))

print(xtable(anova_stats(maov), caption = tab(paste0("ANOVA for long-term marketing mix elasticity for ", names(ordered_vars)[match(var, ordered_vars)], ' ', tablelabel, ''))),type='html',caption.placement='top')
}
...
```{r newanalysis_nomc, results = 'asis', include = TRUE, echo=FALSE}

#re-meancenter SBBE

# formulas for models
upd_models = list(m1=.~1+ emerging,
                  m2 = .~1 + emerging + sbbe_mc + I(sbbe_mc*emerging))

out1=regmodel(formula=c(upd_models), dat=elast, model='lm')

upd_models2 = list(m3 = .~1 + emerging + sbbe_mc + I(sbbe_mc*emerging)+(1|country) + (1|category) + (1|brand),
                   m4 = .~1 + emerging + sbbe_mc + I(sbbe_mc*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                   m5 = .~1 + ln_gci_00.03_gdppercap_s_mc + sbbe_mc + I(sbbe_mc*ln_gci_00.03_gdppercap_s_mc)+ (1|brand) + (1|category) + (1|country) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                   m6 = .~1 + ln_gci_overall_s_mc + sbbe_mc + I(sbbe_mc*ln_gci_overall_s_mc)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                   m7 = .~1 + ln_hdi2010_mc + sbbe_mc + I(sbbe_mc*ln_hdi2010_mc)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                   m8 = .~1 + emerging + brandz + I(brandz*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                   m9 = .~1 + emerging + brandz_global_alltime + I(brandz_global_alltime*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                   m10 = .~1 + emerging + brandz_financial500 + I(brandz_financial500*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market
...
covars = c("(Intercept)", "emerging", "ln_gci_00.03_gdppercap_s_mc","ln_gci_overall_s_mc", "ln_hdi2010_mc",
  "sbbe_mc", "brandz", 'brandz_global_alltime', 'brandz_financial500',
  "I(sbbe_mc * emerging)",
  "I(sbbe_mc * ln_gci_00.03_gdppercap_s_mc)",
  "I(sbbe_mc * ln_gci_overall_s_mc)",
  "I(sbbe_mc * ln_hdi2010_mc)",
  "I(brandz * emerging)",
  "I(brandz_global_alltime * emerging)",
  "I(brandz_financial500 * emerging)",
  "ln_herf_mc",
...
printout(outx, 'lt', title = tab('Regression with long-term elasticities - with non-standardized SBBE'), vars=ordered_vars[1:2],  notes=gsub('long[-]term', 'short- and long-term', notes_new), covariate_choices = covars)

cat("<P style='page-break-before: always'>")
printout(outx, 'lt', title = tab('Regression with long-term elasticities - with non-standardized SBBE'), vars=ordered_vars[3:4],  notes=gsub('long[-]term', 'short- and long-term', notes_new), covariate_choices = covars)
...
Brand equity (top brand) | Indicator variable, whether brand has been listed in BrandZ's Global Top 100 or China Top 50 in 2010.
...
covars <- c('sbbe_std', 'brandz','herf', 'market_growth', 'appliance', 'gini', 'local_to_market')
...
out2=regmodel(formula=c(main=form_main_model_selected_interact), dat=elast, model='lmer')
# keep only models that belong to one type (so that each table only features the rows that were actually estimated)
main_interact = lapply(out2, function(x) {x$lt <- list(x$lt$main); x$st <- list(x$st$main); x})
...
# MAIN MODEL WITH SELECTED INTERACTIONS
cat("<P style='page-break-before: always'>")
printout(main_interact, 'stlt', title = tab('Regression with short- and long-term elasticities in a pooled sample of developed and emerging countries, with selected interactions'), vars=ordered_vars,  notes=gsub('long[-]term', 'short- and long-term', notes_base))


```

```{r main_regs_both, echo = FALSE, results='asis', include=TRUE}
...
for (elasttype in c('st', 'lt')) {
  ...
  #sink('../temp/test.html')
  cat("<P style='page-break-before: always'>")
  printout(mod_out, elasttype, title = tab(paste('Regression with ', ifelse(elasttype=='st', 'short-term', 'long-term'), ' elasticities in a pooled sample of developed and emerging countries: main model, model with full interaction terms, and model with model-based selected interaction terms')), vars=ordered_vars, notes=paste0(ifelse(elasttype=='st', gsub('long[-]term', 'short-term', notes_base), notes_base),  ' For each variable, the first column reports a model without interactions (our main model); the second column reports a model with full interactions; the third column reports a model in which only interactions are added that were significant in an auxilary regression with all main effects, and only one of the relevant interaction terms each.'))
  #sink()
...
modellist2 = lapply(c('developed', 'ln_gci_00.03_gdppercap_s_mc', 'ln_gci_overall_s_mc', 'ln_hdi2010_mc', 'gcifactor_mc'), 
                   function(x) update.formula(update.formula(form_main_model_selected_interact, .~.-emerging), as.formula(paste0('.~. +', x))))

out1=regmodel(formula=modellist, dat=elast, model='lmer')

out2=regmodel(formula=modellist2, dat=elast, model='lmer')
...
cat("<P style='page-break-before: always'>")
printout(out2, 'lt', vars=ordered_vars[1:2], title = tab('Regression with long-term elasticities on covariates and different indicators for a country\'s state of development, with selected interactions'),  notes=notes)

cat("<P style='page-break-before: always'>")
printout(out2, 'lt', vars=ordered_vars[3:4], title = tab('Regression with long-term elasticities on covariates and different indicators for a country\'s state of development, with selected interactions'),  notes=notes)
#sink()
...
shifter = lapply(main, function(i) {
  ...
  shifter = coefs['(Intercept)',]$Estimate+coefs['appliance',]$Estimate*means['appliance']
  return(shifter)
})
...
sims=rbindlist(lapply(names(main), function(mod) {
  simdata[, form:=paste0(shifter[mod], '+emerging*', emerging,'+local_to_market*', local_to_market,'+sbbe_std_mc*', modvar)]
...
p<-ggplot(simtmp, aes(x=l_emerging,y=est))+geom_bar(stat="identity")+facet_grid(l_brandequity~l_local, switch='y') + ggtitle(paste0(rename.fkt(unique(simtmp$model)), ' elasticities')) + xlab('Development status') + ylab('Mean elasticity')
...
## Means of marketing mix instruments per brand

...to illustrate whether level effects may drive our effects.

```{r plotvars,echo=FALSE, include=FALSE}
...
.vars=c('rwpspr','wpswdst','llen','nov12sh')

for (i in .vars) {
  par(mfrow=c(1,3))
  dic=c('../../post-analysis/code/renaming.txt')
  
  # by country
  tmp=elast[!is.na(elastlt)&variable==i]
  
  tmp_bycntry=tmp[,list(w_elast = sum(elastlt/elastlt_se)/sum(1/elastlt_se)), by=c('variable','country')]
  setorderv(tmp_bycntry, 'w_elast', ifelse(i=='rwpspr', 1,-1))
  
	tmp_bycat=tmp[, list(w_elast = sum(elastlt/elastlt_se)/sum(1/elastlt_se)), by=c('variable','category')]
	setorderv(tmp_bycat, 'w_elast', ifelse(i=='rwpspr', 1,-1))
	
	tmp_bybrand=tmp[, list(w_elast = sum(elastlt/elastlt_se)/sum(1/elastlt_se)), by=c('variable','brand')]
  ...
  elast_name=paste0(rename.fkt(i, dictionary=dic), ' elasticities')
  
  barplot(tmp_bycntry$w_elast,names.arg=rename.fkt(tmp_bycntry$country, dictionary=dic), las=2, cex.names=.8, main = fig(paste0(elast_name, ', by countries')),cex.main=.8, ylim = c(ymin, ymax), ylab = 'Long-term elasticity (weighted by inv. SEs)')
	barplot(tmp_bycat$w_elast,names.arg=rename.fkt(tmp_bycat$category, dictionary=dic), las=2, cex.names=.8, main = fig(paste0(elast_name, ', by categories')),cex.main=.8, ylim = c(ymin, ymax), ylab = 'Long-term elasticity (weighted by inv. SEs)')
	barplot(tmp_bybrand$w_elast,names.arg=NULL, las=2, cex.names=.8, main = fig(paste0(elast_name, ', by brands')),cex.main=.8, ylim = c(ymin, ymax), ylab = 'Long-term elasticity (weighted by inv. SEs)')
	
	hist(tmp_bycntry$w_elast, las=2,main = fig(paste0(elast_name, ', by countries')), xlab = 'Long-term elasticity (weighted by inv. SEs)')
	hist(tmp_bycat$w_elast, las=2,main = fig(paste0(elast_name, ', by categories')), xlab = 'Long-term elasticity (weighted by inv. SEs)')
	hist(tmp_bybrand$w_elast, las=2,main = fig(paste0(elast_name, ', by brands')), xlab = 'Long-term elasticity (weighted by inv. SEs)')
...
for (i in .vars) {
  ...
  print(xtable(anova_stats(m), caption = tab(paste0("ANOVA for long-term marketing mix elasticity for ", rename.fkt(i,dic), ''))),type='html',caption.placement='top')
}
```