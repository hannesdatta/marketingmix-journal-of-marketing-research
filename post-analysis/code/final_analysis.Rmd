...
if (estimtype=='weighted') {
  ...
  for (.var in c('elast', 'elastlt')) {
    ...
    elast_simulated[!is.na(get(.var)), paste0('w_', .var) := 1/get(paste0(.var, '_se'))]
    # rescale
    elast_simulated[!is.na(get(.var)), paste0('w_', .var) := get(paste0('w_', .var))/max(get(paste0('w_', .var)))]
    elast_simulated[!is.na(get(.var)), paste0('z_', .var) := get(.var)/get(paste0(.var, '_se'))]
...
```{r analysis_for_all_mmix_slides, results = 'asis', include = T, echo=FALSE}
...
printout(out1, 'lt', title = tab(paste0('Regression with long-term elasticities'), prefix=''), vars=ordered_vars,  notes=paste0(gsub('long[-]term', 'short- and long-term', notes_new), ''), covariate_choices = covars)

```

```{r plot_be_interaction, fig.width=8,fig.height=6,echo=F}
...
covars_summary <- c('usalessh', ordered_vars)
...
upd_models = list(m1 = .~1 + emerging + sbbe_std_mc + I(sbbe_std_mc*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                  ...
                  m3x = .~1 + emerging + sbbe_std_mc + I(sbbe_std_mc*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market + international
                  )

out=regmodel(formula=c(upd_models), dat=elast_backup, model='lmer')

outlagx = regmodel(formula=list(m1=upd_models$m1), dat=elast_laggedx, model='lmer')

elast_backup[is.na(gamma_lagms), gamma_lagms:=0]
elast_backup[, elastlt := beta/(1-gamma_lagms)]

out2=regmodel(formula=upd_models, dat=elast_backup, model='lmer')


outsim=regmodel(formula=list(m1=upd_models$m1), dat=elast_simulated, model='lmer')

outx = NULL

  for (mod in names(out)) {
    outx[[mod]]$variable=out[[mod]]$variable
    outx[[mod]]$st=c(out[[mod]]$st$m1, out[[mod]]$st$m3, out2[[mod]]$st$m1, out[[mod]]$st$m3x, outlagx[[mod]]$st$m1, outsim[[mod]]$st$m1)
    outx[[mod]]$lt=c(out[[mod]]$lt$m1, out[[mod]]$lt$m3, out2[[mod]]$lt$m1, out[[mod]]$lt$m3x, outlagx[[mod]]$lt$m1, outsim[[mod]]$st$m1)
  }
...
for (i in seq(along=ordered_vars)) {
  ...
  printout(outx, 'lt', title = tab(paste0('Robustness checks for long-term ', tolower(gsub('ity', 'ities', gsub('Line length', 'Line-length', names(ordered_vars[i])))), ''), prefix='A'), vars=ordered_vars[i],  notes=paste0(gsub('term elasticities', 'term elasticities (models 1-2, 4; beta/(1-gamma) for model 3)', notes_base), ''), covariate_choices = covars, table.layout.overwrite=c('-c#m-t-a-n'),
           colnames_overwrite=c('Main model', 'Controlling for marketshare', 'DV is beta/(1-gamma)' ,'Controlling for international brands', 'Marketing-mix and brand-specific dynamics', 'Simulation'), colsep_overwrite = c(1,1,1,1,1))
}
