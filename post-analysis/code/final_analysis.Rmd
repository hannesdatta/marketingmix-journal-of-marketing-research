...
Price                | Sales-weighted^1^ average of each SKU's price sold by brand *i* in period *t*, expressed in local currencies and adjusted by a country's consumer price index^2^
...
New product activity | Number of SKUs introduced by brand *i* in the most recent quarter (*t-2*, *t-1*, *t*), expressed as a share of total unique SKUs sold by brand *i* over the same period
...
------------------------- | ------------------- | ---------------------------------------------|----------
appliance                 | Appliance (vs. electronics)| Dummy, equaling 1 for appliances (washing machines, microwaves and refridgerators), 0 if not.
growth_rate               | Category growth rate       | Log of a market's average growth rate in the observation period (geometric average)
...
# interaction model
m2 = ~1+ (1|brand) + (1|category) + (1|country) + necessity*developed + ln_market_growth_mc*developed + ln_herf_mc*developed + local_to_market*developed + western_brand*developed + sbbe_std_mc*developed
cat("<P style='page-break-before: always'>")
...
tmp=cor(as.matrix(tmp))
tmp=tmp[-c(1:4),c(1:4)]
tmp=sanitize_table(tmp)

stargazer(tmp, type='html', title = tab('Correlations of infrastructure pillars with marketing-mix elasticities'),
          initial.zero=FALSE, notes = 'Table reports the correlations between the infrastructure pillars of the Global Competititveness Report (2010/2011 data, see https://www.weforum.org/reports/global-competitiveness-report-2010-2011), and country-fixed effects in a regression of marketing mix elasticities on country-fixed effects, random effects for brands and categories, as well as moderators. Weights correspond to inverse standard errors of the estimated elasticities.')
