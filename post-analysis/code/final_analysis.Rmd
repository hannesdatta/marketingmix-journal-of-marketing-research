...
kable(tmp, format='html', format.args = list(big.mark = ","), initial.zero = FALSE, caption=tab('Development indicators for countries in sample^1^')) %>%
...
Variable                  | Operationalization                                    
-------------------- | ------------------------------------------------------------------
Brand strength       | Sales-based brand equity, derived from the market share attraction model, standardized by category.
...
^3^ All continuous variables are first logged and then mean-centered before model estimation (except brand strength, which is only mean-centered as this variable can take on negative values). 

^4^ Geometric growth rate is the n-th's square root of the ratio of the total number of unit sales in a market in the last year over total number of unit sales in the first year (both years having 12 months of data), where n is equal to the number of years in the observation window.
...
```{r elasts_full, echo=F,results='asis'}
  ...
  out = lapply(list(st=c('elast', 'z_elast', 'w_elast'), lt = c('elastlt','z_elastlt','w_elastlt')), function(obj) {
    ...
    tmp=tmp[, list(Neffects=.N, median_elast = median(val), 
												...
												by=c('variable')]
 #   paste0('[',paste(elast, formatC(unlist(quantile(elastlt, c(.05,.95)))),collapse=', '),']')),
    ...
    diff_tests=lapply(c(ttest='ttest', wls='wls',re='re'), function(tvalrep) {
      ...
      diff_tests=rbindlist(lapply(unique(elast$variable), function(.var) {
        data.table(variable=.var, effect=c('constant', 'emerging'), summary(lm(val~1+emerging, weights=val_w,data=tmp[variable==.var]))$coefficients)
            ...
            taddition = paste0('Tests on differences carried out by regressing elasticities (', estimnoteshort, ') on an intercept and an emerging country indicator variable. Reported t-value is obtained from the estimate of the emerging country indicator.')
      ...
      # random effects based p values
        ...
        taddition = paste0('Tests on differences carried out by regressing elasticities (', estimnoteshort, ') on an intercept, an emerging country indicator variable and three random effects, for brands, categories, and countries, respectively. Reported t-value is obtained from the estimate of the emerging country indicator.')
...
notes_new = paste0("", notes_base, ' Continuous variables have been mean-centered. Lok-likelihood ratio tests computed relative to preceding models.')
...
for (i in 1:4) {
  ...
  printout(outx, 'lt', title = tab(paste0('Regression with long-term ', gsub('ity', 'ities', names(ordered_vars[i])))), vars=ordered_vars[i],  notes=paste0(gsub('long[-]term', 'short- and long-term', notes_new), ''), covariate_choices = covars)
}
#sink()
...
out <- lapply(ordered_vars, function(mod) {
  ...
  p<-ggplot(simtmp, aes(x=l_brandequity,y=est, fill = l_emerging)) + theme_tufte() +geom_bar(stat="identity", position = 'dodge', width= .5) + ggtitle(paste0(rename.fkt(mod), ' elasticities')) + xlab('Brand strength') + ylab('Mean elasticity') + labs(fill="Country\'s degree of development") + scale_fill_grey(start = 0, end = .9)
  return(p)

})

p <- ggarrange(plotlist=out, nrow=2, ncol=2, common.legend=TRUE, legend='bottom')
annotate_figure(p, top=fig('Elasticity plots'), bottom = text_grob('Note: Plots show estimated long-term marketing mix elasticities (weighted by inverse standard errors), using a median split on brand strength\nand a country\'s degree of development.', size= 8, just='left', x = 0))
```
...
attributes <- suppressWarnings(melt(brand_panel[, c('category', grep('^attr', colnames(brand_panel),value=T)),with=F], id.vars=c('category')))

attributes[, na:=all(is.na(value)),by=c('category','variable')]
nobs=nrow(attributes)

attr = attributes[na==F, list(mean=mean(value), sd=sd(value), min=min(value), max=max(value)),by=c('category','variable')]
...
print(kable(tabl, digits=3, caption = tab('Overview of physical search attributes per category^1^', prefix='A')) %>% kable_styling() %>% footnote(number=c(paste0('Computed across ', prettyNum(nobs, big.mark = ','), ' observations in our sample.'))))
```
...
covars <- c('usalessh', 'rwpsprd', 'wpswdst', 'llen', 'nov12sh')
...
tmp=tmp[, lapply(.SD, function(x) c(mean=mean(x,na.rm=T),sd=sd(x,na.rm=T),min=min(x,na.rm=T), max=max(x,na.rm=T))), .SDcols=c(covars)]


tmp[, var:=rep(c('mean', 'sd','min', 'max'),1)]
tmp=melt(tmp, id.var=c('var'))

dtf=dcast(tmp, variable~var)
setcolorder(dtf, c('variable', rep(c('mean', 'sd', 'min','max'),1)))

dtf=sanitize_table(dtf)


print(kable(dtf, format='html', initial.zero = FALSE, digits=3,caption = tab(paste0('Summary statistics for variables in market share attraction model^1^'),prefix='A')) %>% kable_styling() %>% footnote(number=c(paste0('Summary statistics for variables (unlogged and not mean-centered), computed across ', prettyNum(nobs, big.mark = ','), ' observations in our sample.'),'Converted to USD.')))
...
printtmp=tmp[, grep('elastlt[_]', colnames(tmp),value=T) ,with=F] #mc[_]
setnames(printtmp, gsub('elastlt[_]','',colnames(printtmp))) #mc[_]
...
notes_new = paste0("", notes_base, ' Continuous variables have been mean-centered.')
...
for (i in 1:4) {
  ...
  printout(outx, 'lt', title = tab(paste0('Robustness check for ', gsub('ity', 'ities', names(ordered_vars[i])), ': The role of market share'), prefix='A'), vars=ordered_vars[i],  notes=paste0(gsub('long[-]term', 'short- and long-term', notes_new), ''), covariate_choices = covars, table.layout.overwrite=c('-c#m-t-a-n'),
...
notes_new = paste0("", notes_base, ' Continuous variables have been mean-centered. Lok-likelihood ratio tests computed relative to preceding models. Model includes lagged values of a brand\'s marketing mix to allow for differential carry-over effects for each marketing mix instrument. Brand-specific trend variables are excluded.')
...
for (i in 1:4) {
  ...
  printout(outx, 'lt', title = tab(paste0('Regression with long-term ', gsub('ity', 'ities', names(ordered_vars[i])), ' in an alternative model with lagged marketing mix instruments')), vars=ordered_vars[i],  notes=paste0(gsub('long[-]term', 'short- and long-term', notes_new), ''), covariate_choices = covars)
}
#sink()
...
mainnote = paste0('We add to our main model the advertising expenditures for ', stats_brands, ' brands, and re-estimate marketing mix elasticities in those markets for which advertising data is available to us (in total ', stats_markets, ' markets from ', stats_categories, ' categories in two countries, China and Hong Kong). In the table above, we compare the marketing mix elasticities from this model, to those estimated from our main model; in the comparison, we only include those ', stats_brands, ' brands for which advertising expenditures are available.')
