...
libs_to_load <- c('data.table', 'stargazer', 'knitr', 'xtable', 'car', 'kableExtra', 'stringi', 'ggplot2', 'ggpubr', 'plyr', 'gridExtra', 'grid', 'lme4') #'blme' 
...
```{r testconvergence, eval=FALSE, echo=FALSE, include=FALSE}
obj=c('elast', 'w_elast', 'z_elast')
...
tmp = elast[!is.na(get(obj[1]))][, ':=' (val=get(obj[1]), 
    ...
    # random effects based p values
        diff_tests=rbindlist(lapply(unique(tmp$variable), function(.var) {
          #print(.var)
        data.table(variable=.var, effect=c('constant', 'emerging'), summary(lmer(val~1+(1|category) + (1|country) + (1|brand) + emerging, weights=val_w,REML=FALSE,control = lmerctrl, data=tmp[variable==.var]))$coefficients)[, p:=2*(1-pnorm(abs(get('t value'))))]
      }))
   
 obj=c('elastlt','w_elastlt', 'z_elastlt')
       
obj=c('elastlt','w_elastlt', 'z_elastlt')
#summary(blmer(val~1+(1|category) + (1|country) + (1|brand) + emerging, weights=val_w,data= tmp[variable=='llen'],control = lmerControl(optimizer ="Nelder_Mead")))


#summary(lmer(val~1+(1|category) + (1|country) + (1|brand) + emerging, weights=val_w,data= tmp[variable=='llen'], REML = FALSE))


#summary(lmer(val~1+(1|category) + (1|country) + (1|brand) + emerging, weights=val_w,data= tmp[variable=='nov12sh'], REML = FALSE))


# one with singular fit
...
tmp = elast[!is.na(get(obj[1]))][, ':=' (val=get(obj[1]), 
                                       val_w=get(obj[2]),
                                       val_z=get(obj[3]))]
#m<-lmer(val~1+(1|category) + (1|country) + emerging, data= tmp[variable=='rwpspr'])


#summary(m)
#m2<-blmer(val~1+(1|category) + (1|country) + emerging, data= tmp[variable=='rwpspr'], cov.prior=wishart)
#summary(m2)
...
#singluar fit
#lmer(elast~1+ (1|brand) + (1|category) + (1|country) + emerging + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + ln_overall_prindexavg_mc + gini_mc + sbbe_std_mc,data=elast[variable=='rwpspr'],weights=1/elast_se)

#lmer(elastlt~1+ (1|brand) + (1|category) + (1|country) + emerging + appliance + ln_market_growth_mc + ln_herf_mc + local_to_market + ln_overall_prindexavg_mc + gini_mc + sbbe_std_mc,data=elast[variable=='llen'],weights=1/elastlt_se)
...
Gini index                | Gini index in a country.
...
covars <- c('sbbe_std', 'brandz','local_to_market', 'herf', 'market_growth', 'appliance', 'overall_prindexavg', 'gini')
