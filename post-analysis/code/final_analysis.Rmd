...
elast[, ':=' (l_emerging=factor(ifelse(emerging==1,em_levels[1],em_levels[2]), levels=em_levels),
             l_brandequity=factor(as.character(ifelse(sbbe_std_mc>median(sbbe_std_mc,na.rm=T), 'strong brand','weak brand')), levels=c('weak brand','strong brand'))),by=c('variable','category','country','market_id')]
...
out <- lapply(ordered_vars, function(mod) {
 ...
 # theme(plot.margin=unit(c(.6,.5,.6,.5),"cm"))
  png(paste0('../output/elasticity_plots_', mod,'.png'), res=200, units='in', height=2.5, width=4)
...
full_plot = annotate_figure(p, top=fig('Elasticity plots'), bottom = text_grob('Note: Plots show estimated long-term marketing-mix elasticities (weighted by inverse standard errors), based on a country\'s degree of development (DM versus EM), and a median split (by category) on brand equity.', size= 8, just='left', x = 0))
full_plot

png('../output/elasticity_plots.png', res=200, units='in', height=8, width=6)
print(p)
dev.off()
