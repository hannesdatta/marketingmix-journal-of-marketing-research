...
for (iter in iters) {
  print(kable(sanitize_table(out[[iter[1]]]), digits=3, format='html', 
        ...
        footnote(general=paste0('*p*<.001 and *p*<.01 (two-sided). ', estimnote, ' Test on differences carried out using WLS by regressing elasticities on an intercept and an emerging country indicator variable. Reporting t-value for estimate of the emerging country indicator.')) %>% add_header_above(c(" " = 1, "All countries" = 4, "Developed countries" = 4, "Emerging countries" = 4, 'Test on differences' =2)))
}
