...
if (estimtype=='weighted') {
  ...
  for (.var in c('elast', 'elastlt')) {
    ...
    elast_adv[!is.na(get(.var)), paste0('w_', .var) := 1/get(paste0(.var, '_se'))]
    # rescale
    elast_adv[!is.na(get(.var)), paste0('w_', .var) := get(paste0('w_', .var))/max(get(paste0('w_', .var)))]
    elast_adv[!is.na(get(.var)), paste0('z_', .var) := get(.var)/get(paste0(.var, '_se'))]
    
    elast_laggedx[!is.na(get(.var)), paste0('w_', .var) := 1/get(paste0(.var, '_se'))]
    # rescale
    elast_laggedx[!is.na(get(.var)), paste0('w_', .var) := get(paste0('w_', .var))/max(get(paste0('w_', .var)))]
    elast_laggedx[!is.na(get(.var)), paste0('z_', .var) := get(.var)/get(paste0(.var, '_se'))]
...
covars = c("(Intercept)", 
           "emerging", "ln_gci_00.03_gdppercap_s_mc","ln_gci_overall_s_mc", "ln_hdi2010_mc",
           "sbbe_std_mc",
...
New product activity | Number of SKUs introduced by brand *i* in a rolling window of one year (*t-11*, *t-10*, *...*, *t*), expressed as share of the total number of unique SKUs sold by brand *i* over the same period (0 = none of the SKUs is new, 100 = all of the SKUs are new).
...
Appliance (vs. electronics)| Dummy variable, equaling 1 for appliances (washing machines, microwaves and refrigerators), 0 for all other categories.
...
# formulas for models
upd_models = list(m1 = .~1 + (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                   m2 = .~1 + emerging + (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                   m3 = .~1 + emerging + sbbe_std_mc + (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
...
llratio <- function(tmp) {
  indx=seq(from=2, length.out=length(tmp)-1)
  annot=sapply(indx, function(i) {
    m<-anova(tmp[[i-1]], tmp[[i]], test ='Chisq')
    c(chi=m$Chisq[2], df=diff(m$Df), p=m$`Pr(>Chisq)`[2])
  })
  
  llratio = c('', apply(annot, 2, function(x) paste0('Chi-sq: ', formatC(x['chi'],digits=3, big.mark=',',format ='f', flag='#'), ', df = ', x['df'], ' (p ', ifelse(x['p']<.001, '< 0.001', paste0('= ',formatC(x['p'], digits=3, format = 'f', flag='#'))), stri_trim(paste0(' ', psignstars(x['p']), ')')))))
  return(llratio)
}

outx = NULL

for (out in list(out1)) {
...
for (var_i in seq(along=outx)) {
  outx[[var_i]]$llratio <- list(st = llratio(outx[[var_i]]$st),
                                lt = llratio(outx[[var_i]]$lt))
                                             
}

for (i in 1:4) {
  cat("<P style='page-break-before: always'>")
  printout(outx, 'lt', title = tab(paste0('Regression with long-term ', gsub('ity', 'ities', tolower(names(ordered_vars[i]))))), vars=ordered_vars[i],  notes=paste0(gsub('explanatory variables', 'explanatory variables (only control variables in model 1; models 2-4 add the emerging-market indicator, brand equity, and their interaction term, respectively),', notes_base), ' ', notes_loglik), covariate_choices = covars)
}
```



```{r plot_be_interaction, echo=F}
...
out <- lapply(ordered_vars, function(mod) {
  ...
  p<-ggplot(simtmp, aes(x=l_brandequity,y=est, fill = l_emerging)) + theme_tufte() +geom_bar(stat="identity", position = 'dodge', width= .5) + ggtitle(paste0(gsub('Line length', 'Line-length', rename.fkt(mod)), ' elasticity')) + xlab('Brand equity') + ylab('Mean elasticity') + labs(fill="Country\'s degree of development") + scale_fill_grey(start = 0, end = .9)
  return(p)

})

p <- ggarrange(plotlist=out, nrow=2, ncol=2, common.legend=TRUE, legend='bottom')
annotate_figure(p, top=fig('Elasticity plots'), bottom = text_grob('Note: Plots show estimated long-term marketing-mix elasticities (weighted by inverse standard errors),\nbased on a country\'s degree of development (DM versus EM), and a median split on brand equity.', size= 8, just='left', x = 0))
```
...
kable(tmp, caption = tab('Country-of-origins for brands in sample, ordered by the number of brands from a given country^1^', prefix ='A'),format='html') %>%
        kable_styling() %>% footnote(number=paste0('Brand names per country are listed alphabetically.'))
...
covars_summary <- c('usalessh', 'rwpsprd', 'wpswdst', 'llen', 'nov12sh')
...
tmp=tmp[, lapply(.SD, function(x) c(mean=mean(x,na.rm=T),sd=sd(x,na.rm=T),min=min(x,na.rm=T), max=max(x,na.rm=T))), .SDcols=c(covars_summary)]


tmp[, var:=rep(c('meancap', 'sdcap','mincap', 'maxcap'),1)]
tmp=melt(tmp, id.var=c('var'))

dtf=dcast(tmp, variable~var)
setcolorder(dtf, c('variable', rep(c('meancap', 'sdcap', 'mincap','maxcap'),1)))

dtf=sanitize_table(dtf)


print(kable(dtf, format='html', initial.zero = FALSE, digits=3,caption = tab(paste0('Summary statistics for variables in market share attraction model^1^'),prefix='A')) %>% kable_styling() %>% footnote(number=c(paste0('Summary statistics for variables (prior to the log-operation and mean-centering) are computed across ', prettyNum(nobs, big.mark = ','), ' observations in our sample.'),'Converted to USD.')))
...
```{r newanalysis_alternatives_development, results = 'asis', include = TRUE, echo=FALSE}
# formulas for models
upd_models2 = list(m5 = .~1 + emerging + sbbe_std_mc + I(sbbe_std_mc*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
...
notes_new = sub('explanatory variables', 'explanatory variables (replacing the emerging-market indicator with alternative metrics for a country\'s development degree)', notes_base)

#sink('test.htm')

for (i in 1:4) {
  cat("<P style='page-break-before: always'>")
  printout(outx, 'lt', title = tab(paste0('Robustness check with long-term ', tolower(gsub('ity', 'ities', gsub('Line length','Line-length', names(ordered_vars[i])))), ' and alternative metrics for a country\'s development degree'), prefix='A'), vars=ordered_vars[i],  notes=notes_new, covariate_choices = covars)
}
#sink()
```
...
upd_models = list(m1 = .~1 + emerging + sbbe_std_mc + I(sbbe_std_mc*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market,
                  m3 = .~1 + emerging + sbbe_std_mc + I(sbbe_std_mc*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market + mean_ms_mc)
...
out2=regmodel(formula=c(upd_models), dat=elast_backup, model='lmer')

outx = NULL

  for (mod in names(out)) {
    outx[[mod]]$variable=out[[mod]]$variable
    outx[[mod]]$st=c(out[[mod]]$st$m1, out[[mod]]$st$m3, out2[[mod]]$st$m1)
    outx[[mod]]$lt=c(out[[mod]]$lt$m1, out[[mod]]$lt$m3, out2[[mod]]$lt$m1)
  }
...
for (i in 1:4) {
  ...
  printout(outx, 'lt', title = tab(paste0('Robustness check for long-term ', tolower(gsub('ity', 'ities', gsub('Line length', 'Line-length', names(ordered_vars[i])))), ': The role of market share'), prefix='A'), vars=ordered_vars[i],  notes=paste0(gsub('term elasticities', 'term elasticities (models 1 and 2; beta/(1-gamma) for model 3)', notes_base), ''), covariate_choices = covars, table.layout.overwrite=c('-c#m-t-a-n'),
           colnames_overwrite=c('Long-term elasticity', 'Long-term elasticity', 'beta/(1-gamma)'), colsep_overwrite = c(1,1,1))
}
...
```{r newanalysis_elastlaggedX, results = 'asis', include = TRUE, echo=FALSE}
...
out1=regmodel(formula=c(upd_models), dat=elast_laggedx, model='lmer')
...
notes_new = paste0(notes_base, ' Log-likelihood ratio tests computed relative to preceding models. Model includes lagged values of a brand\'s marketing mix (instead of a brand\'s lagged market share) to allow for differential carry-over effects for each marketing-mix instrument.')
...
for (i in 1:4) {
  ...
  printout(outx, 'lt', title = tab(paste0('Regression with long-term ', gsub('ity', 'ities', tolower(gsub('Line length', 'Line-length', names(ordered_vars[i])))), ' in an alternative model with lagged marketing-mix instruments'), prefix='A'), vars=ordered_vars[i],  notes=paste0(gsub('long[-]term', 'short- and long-term', notes_new), ''), covariate_choices = covars)
}
...
for (iter in iters) {
  print(kable(sanitize_table(out[[iter[1]]]), digits=3, format='html', format.args = list(big.mark = ","),
        ...
        footnote(number=c(paste0(notes_sig, ' ', mainnote),estimnote, 'Significance tested using meta-analytic p-value.', notes[1])) %>% add_header_above(c(" " = 1, "Advertising included" = 3, "Advertising not included (main model)" = 3, 'Tests on differences' =2)))
}
```

```{r analysis_for_all_mmix_slides, results = 'asis', include = F, echo=FALSE}
...
notes_new = paste0("", notes_base, ' ', notes_loglik)

cat("<P style='page-break-before: always'>")

printout(out1, 'lt', title = tab(paste0('Regression with long-term elasticities'), prefix='A'), vars=ordered_vars,  notes=paste0(gsub('long[-]term', 'short- and long-term', notes_new), ''), covariate_choices = covars)

```

```{r correl_complete, echo=FALSE, results = 'asis'}
brand_panel[, trend:=1:.N,by=c('market_id', 'brand')]
brand_panel[, laggedms:=c(NA, usalessh[-.N]),by=c('market_id', 'brand')]
brand_panel <- brand_panel[!trend==1]

# mean center
printtmp=brand_panel[, c('market_id', ordered_vars,'trend', 'laggedms'), with=F]
printtmp[, llen:=as.double(llen)]
printtmp[, trend:=as.double(trend)]

#for (.var in setdiff(colnames(printtmp), 'market_id')){
#  printtmp[,(.var):=as.double(get(.var)-mean(get(.var),na.rm=T)),by=c('market_id')]
#}
printtmp[, market_id:=NULL]

setnames(printtmp, gsub('elastlt[_]','',colnames(printtmp)))

setnames(printtmp, paste0(rename.fkt(colnames(printtmp)), ''))

tmp2=corstars(printtmp, method=c("pearson"),removeTriangle=c('upper'),result='none')

kable(tmp2, format='html', caption=tab('Correlation among focal covariates',prefix='A')) %>%
        kable_styling() 
...
```{r analysis_international, results = 'asis', include = T, echo=FALSE}

elast[, international:=ifelse(ncountries>=2,1,0)]

upd_models = list(m4 = .~1 + emerging + sbbe_std_mc + I(sbbe_std_mc*emerging)+ (1|country) + (1|category) + (1|brand) + ln_herf_mc + ln_market_growth_mc + appliance + ln_gini_mc + local_to_market + international)
...
printout(out1, 'lt', title = tab(paste0('Robustness check: Controlling for international brands'), prefix='A'), vars=ordered_vars,  notes=paste0(gsub('long[-]term', 'short- and long-term', notes_new), ''), covariate_choices = covars)
