...
# formulas for models
form_main_model = .~1+ (1|brand) + (1|category) + (1|country) + developed + sbbe_std_mc + local_to_market + ln_herf_mc +ln_market_growth_mc +  ln_overall_prindexavg_mc + appliance + gini_mc

form_main_model_selected_interact = .~1+ (1|brand) + (1|category) + (1|country) + developed + sbbe_std_mc + I(sbbe_std_mc*developed) + local_to_market + I(local_to_market*developed) + ln_herf_mc + I(ln_herf_mc*developed) + ln_market_growth_mc + I(ln_market_growth_mc*developed) + ln_overall_prindexavg_mc + appliance + gini_mc

#meanglobalx local_multip_market
#local_multip_market +

form_interact = ~1+ (1|brand) + (1|category) + (1|country) + developed + sbbe_std_mc + I(sbbe_std_mc*developed) + local_to_market + I(local_to_market*developed)  +  ln_herf_mc + I(ln_herf_mc*developed) + ln_market_growth_mc + I(ln_market_growth_mc*developed) + appliance + I(appliance*developed) + ln_overall_prindexavg_mc + I(ln_overall_prindexavg_mc*developed) + gini_mc + I(gini_mc*developed)
...
```{r elast_plots_category, echo=F, fig.height=6, fig.width=10}

out = lapply(ordered_vars, function(var) {
   
    tmp=elast[variable==var, list(elastlt=sum(elastlt*w_elastlt)/sum(w_elastlt)), by = 'category']
    setorder(tmp, elastlt)
    if(var=='rwpspr') tmp=setorderv(tmp, 'elastlt', order=-1L)
    tmp[, cat:=rename.fkt(category, dictionary=c('renaming.txt'))]
    tmp[, cat:=factor(cat, levels=tmp$cat)]
    
    g<-ggplot(tmp, aes(x=cat, y=elastlt, fill = T)) + geom_bar(stat='identity') + coord_flip() + ggtitle(capitalize(gsub('icity', 'icity', names(ordered_vars)[match(var, ordered_vars)]))) + xlab('') + ylab('Elasticity') + scale_fill_manual(values=c('darkblue')) + theme(legend.position='none')
    return(g)   

  }) 

grid.arrange(out[[1]], out[[2]], out[[3]], out[[4]], nrow=2, top = fig('Elasticities by category'), bottom = '')
 
  
```

```{r elast_plots_countries, echo=F, fig.height=10}
...
out = lapply(ordered_vars, function(var) {
    ...
    g<-ggplot(tmp, aes(x=cat, y=elastlt, fill = country_class)) + geom_bar(stat='identity') + coord_flip() + ggtitle(capitalize(gsub('icity', 'icity', names(ordered_vars)[match(var, ordered_vars)]))) + xlab('') + ylab('Long-term elasticity') + scale_fill_manual(values=c('darkblue','red'))+ theme(legend.position='none', axis.text.x=element_text(angle=45, hjust=1,size=12)) 
  ...
  for (i in 1:2) {
    ...
    diff_tests=lapply(c(ttest='ttest', wls='wls',re='re'), function(tvalrep) {
      ...
      diff_tests=rbindlist(lapply(unique(elast$variable), function(.var) {
        data.table(variable=.var, effect=c('constant', 'developed'), summary(lm(val~1+developed, weights=val_w,data=tmp[variable==.var]))$coefficients)
      }))
            taddition = paste0('Test on differences carried out by regressing elasticities (', estimnoteshort, ') on an intercept and a developed country indicator variable. Reported t-value is obtained from the estimate of the developed country indicator.')
      ...
      # random effects based p values
        ...
        data.table(variable=.var, effect=c('constant', 'developed'), summary(lmer(val~1+(1|category) + (1|country) + (1|brand) + developed, weights=val_w,control = lmerctrl, data=tmp[variable==.var]))$coefficients)[, p:=2*(1-pnorm(abs(get('t value'))))]
      }))
        taddition = paste0('Test on differences carried out by regressing elasticities (', estimnoteshort, ') on an intercept, a developed country indicator variable and three random effects, for brands, categories, and countries, respectively. Reported t-value is obtained from the estimate of the developed country indicator.')
    ...
    # random effects based p values
      diff_tests=rbindlist(lapply(unique(tmp$variable), function(.var) {
        xemerg=tmp[variable==.var&developed==1]$val
        xdevel=tmp[variable==.var&developed==0]$val
        
        tout = t.test(xemerg,xdevel)
        
      data.table(variable=.var, effect=c('developed'), NA, NA, tout$statistic, tout$p.value)
      }))
    ...
    diff_tests=diff_tests[effect=='developed'][, effect:=NULL]
...
tmp = unique(tmp, by = c('market_id','brand', 'developed'))
nobs=nrow(tmp)

# test for differences in means
diff_tests=rbindlist(lapply(covars, function(.var) {
      tmp[, val:=get(.var)]
      data.table(variable=.var, effect=c('constant', 'developed'), summary(lm(val~1+developed,data=tmp))$coefficients)
    }))
...
tmp=tmp[, lapply(.SD, function(x) c(mean=mean(x,na.rm=T),sd=sd(x,na.rm=T),min=min(x,na.rm=T), max=max(x,na.rm=T))), by = c('developed'), .SDcols=c(covars)]
tmp[, var:=rep(c('mean', 'sd','min', 'max'),2)]
tmp=melt(tmp, id.var=c('developed', 'var'))
tmp[, developed_label:=ifelse(developed==1, 'developed', 'emerging')]

dtf=dcast(tmp, variable~developed_label+var)
setcolorder(dtf, c('variable', paste0(rep(c('developed_', 'emerging_'), each=4), rep(c('mean', 'sd', 'min','max'),2))))
...
print(kable(dtf, format='html', initial.zero = FALSE, digits=3,caption = tab(paste0('Summary statistics for covariates in second-stage analysis, split by a country\'s development status^1^'))
      ) %>% kable_styling() %>% footnote(number=c(paste0('Summary statistics for variables (unlogged and not mean-centered), computed across ', nobs, ' observations in our sample.'),'Test on differences carried out by regressing variables on an intercept and a developed country indicator variable. Reported t-value is obtained from the estimate of the developed country indicator.')) %>% add_header_above(c(" " = 1, "Developed countries" = 4, "Emerging countries" = 4, "Test on differences^2^"=2)))
#sink()
