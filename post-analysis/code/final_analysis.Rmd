...
# check correlation between llen and nov6sh
brand_panel=fread('../../analysis/temp/preclean.csv')
brand_panel[, ':=' (date = as.Date(date))]

with(brand_panel, cor(llen,nov6sh))

tmp=brand_panel[, (cor = cor(llen,nov6sh)), by = c('market_id')]
mean(tmp$V1)


tmp=brand_panel[, list(V1=cor(llen,nov6sh, use = 'pairwise')), by = c('market_id', 'brand')]
...
for (elasttype in c('st', 'lt')) {
  ...
  out1b=regmodel(formula=c(form_interact, run_interact), dat=elast, model='lmer')
