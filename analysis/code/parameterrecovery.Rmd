...
evals <- lapply(list(out1, out2), function(out) {
    
  o = do.call('rbind', out)
  res=colMeans(o)
  
  for (i in 1:4) o[,paste0('lx', i)]<- (-o[,paste0('lx', i)])/o[,'ly']
  true = c(dx1=1, dx2=-2, dx3=.5, dx4=2, lx1=1/lambda, lx2=-2/lambda, lx3=.5/lambda, lx4=2/lambda)
...
```{r}
vars=grep('^dx[0-9]$|lx[0-9]$', colnames(evals[[1]]), value=T)

comp=rbind(data.table(evals[[1]][, vars], type='without Copulas', iteration = 1:nrow(evals[[1]])),
      data.table(evals[[2]][, vars], type='with Copulas', iteration = 1:nrow(evals[[1]])))
tmp=melt(comp, id.vars=c('type'))

tmp2 = tmp[variable%in%vars, list(mean=mean(value)),by=c('type','variable')]

tmp3=cbind(dcast(tmp2, variable~type), true=c(dx1=1, dx2=-2, dx3=.5, dx4=2, lx1=1/lambda, lx2=-2/lambda, lx3=.5/lambda, lx4=2/lambda))
setcolorder(tmp3, c('variable', 'true', 'without Copulas','with Copulas'))
