...
reps = 50
set.seed(1234)
data <- lapply(1:reps, function(...) sim_data(rho=0))

#out=lapply(data, function(dt) lm(y~1+x1+x2+x3+x4+quarter1+quarter2+quarter3+ly,data=dt)$coefficients)

out=lapply(data, function(dt) lm(dy~1+dx1+dx2+dx3+dx4+quarter1+quarter2+quarter3+ly-
                                   lx1-lx2-lx3-lx4,data=dt)$coefficients)
...
# recovery
res[grepl('^l',names(res))]/res[which(names(res)=='ly')]
