...
# Exploration w/ Marnik

```{r, include=TRUE}
tmp=elast_loglog[grepl('pr$',variable)]
#tmp=elast_loglog[grepl('tablets', category)& grepl('pr$',variable)]
setorderv(tmp, 'elastlt', order=-1L)
tmp
```
...
ids = unlist(lapply(results_loglog, function(x) x$elast$brand_id[1]))
bid=1372
id=which(ids==bid)

stargazer(results_loglog[[id]]$model, type='html')
cat(paste0('Dependent variable: ', colnames(results_loglog[[id]]$model$model)[id]), fill =T)
