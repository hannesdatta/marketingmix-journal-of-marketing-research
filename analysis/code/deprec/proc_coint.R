# Procedure that extends the existing ca.jo function to conduct a 
# Johansen Cointegration test by introducing seasonal centred quarterly dummies.

library("urca")

ca.jo2 = function (x, type = c("eigen", "trace"), ecdet = c("none", "const", 
    ...
    dumvar = NULL) 
{
   ...
   if (!is.null(season)) {
	...
	if (!(is.null(season))) {
        dum <- (diag(length(unique(season))) - 1/length(unique(season)))[, -length(unique(season))]
        #dums <- dum
        
		dums <- do.call('rbind', lapply(season, function(i) dum[i,]))
