...
sim_data <- function(rho = .5, N = 1000, pi_vals = c(-.3, .7), coef_x = 3, type='latentIV') {
  ...
  # Step 3: Compute endogenous regressor (uniformly distributed from min(unif) to max(unif)
  ...
  if (type=='copula') {
    ...
    F_x=pnorm(errorterm[,2])
	  x = F_x*(max(pi_vals)-min(pi_vals))+min(pi_vals)
	#F_y=pnorm(y)
	
	
  #x <- runif(N, min = min(pi_vals), max = max(pi_vals)) + errorterm[,2]
  ...
  return(list(data = data.frame(y, x), z=z,pi_vals=pi_vals, coef_x = coef_x))
