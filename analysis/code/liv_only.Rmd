...
nlminb  = nlminb(start = params, objective = llik, reparametrization = TRUE)
  
# with simulated maximum likelihood
MLE_sim = optimx(params, fn = llik, method = 'Nelder-Mead', hessian = FALSE, itnmax = 10000, 
        control = list(trace = 0, dowarn = FALSE), sim=T)
#unlist(MLE_sim)
nlminb_sim  = nlminb(start = params, objective = llik, sim = T, reparametrization = TRUE)
  
ols = lm(y~1+x)

kable(data.frame(ols=ols$coefficients, MLE_neldermead=unlist(MLE)[1:2], `MLE_neldermead_simulatedmaxlik`=unlist(MLE_sim)[1:2],
           MLE_port=unlist(nlminb$par)[1:2], `MLE_port_simulatedmaxlik`=unlist(nlminb_sim$par)[1:2]),
...
out = clusterApplyLB(cl, data, function(dt) {
  ...
  nlminb  = nlminb(start = params, objective = llik, sim = TRUE, reparametrization = TRUE)#,
                   #control=list(abs.tol = 1e-20, rel.tol = 1e-10))
   
                   #control=list(abs.tol = 1e-20, rel.tol = 1e-10))
  
    MLE = optimx(nlminb$par, fn = llik, method = 'Nelder-Mead', hessian = FALSE, itnmax = 10000, 
        control = list(trace = 0, dowarn = FALSE), sim = TRUE,reparametrization = TRUE)
 ...
 # MLE = optimx(params, fn = llik, method = 'Nelder-Mead', hessian = FALSE, itnmax = 10000, 
 #       control = list(trace = 0, dowarn = FALSE), sim = TRUE)
  unlist(MLE)
  #c(nlminb$par, nlminb$convergence, nlminb$objective)
  #c(ucminf$par, ucminf$convergence)
})
...
#3) why can't i specify lambdas with exp (optimizer problem? / label switching)
# --> tried nlminb, and others; they are faster, but sometimes less accurate

# I observed p wasn't changing. keep MLE?

#3) why can i not use better starting values - OLS don't work well!


# 4) Extend to multiple endogenous variables
