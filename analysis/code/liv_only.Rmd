...
data.frame(ols=ols$coefficients, MLE=unlist(MLE)[1:2], `MLE_simulatedmaxlik`=unlist(MLE_sim)[1:2])
...
out = clusterApplyLB(cl, data, function(dt) {
  ...
  params = c(0, 0, 1, 0, 1, 0, 1, 2)
  #params = c(startpar, uppertri,pgo,lambdas)
  
  MLE = optimx(params, fn = llik, method = 'BFGS', hessian = FALSE, itnmax = 10000, 
...
kable(t(apply(res, 2, function(x) return(c(mean=mean(x), sd=sd(x))))), caption = 'Comparison between OLS versus REndo-based LIV estimator')
...
#3) why can't i specify lambdas with exp - optimizer problem!
