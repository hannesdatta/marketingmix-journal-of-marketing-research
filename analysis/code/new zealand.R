...
# deactivate the rest
	...
	maxit=400
	# apple benchmark
	out = analyze_by_market(31, estmethod='FGLS', setup_y = setup_y, setup_x = setup_x, setup_endogenous = setup_endogenous, trend = 'always', pval = pval, max.lag = max.lag, min.t = min.t, maxiter = maxit, use_quarters=T, plusx=plusx, attraction_model=attraction_model, squared=squared, takediff=takediff, lag_heterog=F)

	# samsung benchmark
	out = analyze_by_market(31, estmethod='FGLS', setup_y = setup_y, setup_x = setup_x, setup_endogenous = setup_endogenous, trend = 'always', pval = pval, max.lag = max.lag, min.t = min.t, maxiter = maxit, use_quarters=T, plusx=plusx, attraction_model=attraction_model, squared=squared, takediff=takediff, lag_heterog=F, benchmarkb='samsung')
	
	#use praise winsten
	out = analyze_by_market(31, estmethod='FGLS-Praise-Winsten', setup_y = setup_y, setup_x = setup_x, setup_endogenous = setup_endogenous, trend = 'always', pval = pval, max.lag = max.lag, min.t = min.t, maxiter = maxit, use_quarters=T, plusx=plusx, attraction_model=attraction_model, squared=squared, takediff=takediff, lag_heterog=F, benchmarkb='samsung')
