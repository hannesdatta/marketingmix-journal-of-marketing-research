...
##########################
  ...
  # ESTIMATE SUR
  split_by_market = split(results_salesresponse_max3_p10_cop, estimated_markets$market_id)
  ...
  if(0){
  # run OLS and compare
  sur_res[[28]]$results = model_sur(split_by_market[[28]], maxiter=1)
  
  sur_res[[28]]$results$coefs[[1]]
  
  X=as.matrix(split_by_market[[28]][[1]]$model_matrix)[,-1]
  X=as.matrix(cbind(1,X))
  y=as.matrix(split_by_market[[28]][[1]]$model_matrix)[,1]
  
  cbind(sur_res[[28]]$results$coefs[[1]],solve(t(X)%*%X)%*%(t(X)%*%y))
  
  finaldf = split_by_market[[28]][[1]]$model_matrix
  
  m2 <- lm(as.formula(paste0(colnames(finaldf)[1], '~1+', paste0(colnames(finaldf)[-1],collapse='+'))), data=finaldf)
  
  focal_model = results_salesresponse_max3_p10_cop[[212]]
  focal_model$sur$coefs = data.frame(summary(m2)$coefficients)
  colnames(focal_model$sur$coefs)<- c('coef','se','t','p')
  focal_model$sur$varcovar = vcov(m2)
  
  #model_sur(split_by_market[[1]])
  
  
  model_simulate(focal_model)
  
  focal_models=split_by_market[[28]]
  
  model_sur(focal_models)
  
  
  model_simulate(results_salesresponse_max3_p10_cop[[212]])
  ...
  #sur_res[[28]]$results
  #focal_models = split(results_salesresponse_max3_p10_cop, estimated_markets$market_id)[[3]]
  #test=unlist(lapply(seq(along=focal_models), function(iter) class(try(model_sur(focal_models[-iter]),silent=T))))
  ...
  elast_sum = rbindlist(lapply(sims,function(x) x$elasticities))
  
  elast_sum[abs(elast6)>100]
  
  summary(elast_sum)
  
  #err <- unlist(lapply(sims,class))
  # MAIN RESULTS: results_salesresponse_max3_p10_cop
  
  saveres <- results_salesresponse_max3_p10_cop
  saveres <- lapply(saveres, function(x) {x$dt=NULL;x$model_matrix=NULL;return(x)})
